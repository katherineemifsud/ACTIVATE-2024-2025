    ...
    if BCB_start_list and BCB_stop_list:
        ...
        for idx, t in enumerate(times):
            if BCB_start <= int(t) <= BCB_stop:
                ...
                category = 'Y' if 0 <= lwc_val <= 0.00001 else 'N'
                if category == 'N':
                    found_N_values = True
                for bin_label in range(3, 21):
                    column_name = f'dNdlogD_{bin_label:03d}_FCDP'
...
for i in filtered_indices:
    ...
    if BCB_start_list and BCB_stop_list:
        ...
        FCDP_flight =FCDP[i]
        times = FCDP_flight.Time_Start.values
        effec_diam = FCDP_flight.ED_FCDP.values
        ...
        for idx, t in enumerate(times):
            if BCB_start <= int(t) <= BCB_stop:
                ...
                if 0 <= lwc_val <= 0.00001:
                    LWC_Y.append(lwc_val)
                    # Get the corresponding effective diameter value for LWC_Y
                    ev_val = effec_diam[idx]
                    print("Added to LWC_Y. Corresponding effective diameter value:", ev_val)
                    print("Length of LWC_Y after adding value:", len(LWC_Y))
                    print("Length of EV_Y after adding value:", len(EV_Y))
                    EV_Y.append(ev_val)
                else:
                    LWC_N.append(lwc_val)
                    # Get the corresponding effective diameter value for LWC_N
                    ev_val = effec_diam[idx]
                    EV_N.append(ev_val)
