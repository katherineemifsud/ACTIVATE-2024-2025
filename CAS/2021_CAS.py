...
# save_path = os.path.join(save_dir, "total_Y_concentration_cm3_beforemass.csv")
...
for i in range(len(dates_legs)):
    ...
    CAS_flight = CAS[i]
    CAS_flight['Time_mid'] = pd.to_numeric(CAS_flight['Time_mid'], errors='coerce')
    CAS_flight['LWC_CAS'] = pd.to_numeric(CAS_flight['LWC_CAS'], errors='coerce')
    times_cas = CAS_flight['Time_mid'].values
    lwc_cas = CAS_flight['LWC_CAS'].values
    for j in range(len(BCB_start)):
        start = BCB_start[j]
        end = BCB_stop[j]

        idx = np.where((times_cas >= start) & (times_cas <= end))[0]

        if len(idx) > 0:
            lwc_mean = np.nanmean(lwc_cas[idx])
        else:
            lwc_mean = np.nan

        master_BCB_LWC.append({
            'Date': date,
            'BCB_start': start,
            'BCB_stop': end,
            'LWC_mean': lwc_mean
...
# save_path = os.path.join(save_dir, "BCB_LWC_means_beforemass.csv")
# master_BCB_LWC_df = pd.DataFrame(master_BCB_LWC)
...
plt.ylabel('Frequency of flight legs', fontsize=15, fontweight='bold')
plt.title('Leg average RH November-December 2021', fontweight='bold', fontsize=16)
plt.xticks(fontweight='bold', fontsize=14)
plt.yticks(fontweight='bold', fontsize=14)
plt.show()
# %%
...
# })
# save_dir = "/home/disk/eos4/kathem24/activate/data/2021/CAS"
# save_path = os.path.join(save_dir, "Average_Dry_Size_Distribution_beforemass.csv")
...
plt.title('Ambient Below Cloud Base November-December 2021', fontsize=19, fontweight='bold')
plt.xscale('log')
plt.yscale('log')
...
plt.title('CAS Below Cloud Base Novemeber-December 2021\nContours of Dry Mass', fontsize=19, fontweight='bold')
plt.xscale('log')
plt.yscale('log')
...
# output_path = "Dry_mass_BCB2021_lessthan100massREAL.csv"
...
# with open("CAS_slope_massLE1002021.pkl", "wb") as f:
#     pickle.dump(filtered_slope_mass100gone, f)
# print("Saved CAS slope-filtered dataset.")
# print("Saved to:", os.path.abspath("CAS_slope_massLE1002021.pkl"))
# print("Exists?", os.path.exists("CAS_slope_massLE1002021.pkl"))
#%%
...
print("Final slope/mass legs:", len(filtered_slope_mass100gone))
print("Original concentration legs:", len(total_concentration_cm3))
print("Matched concentration legs:", len(total_concentration_cm3_mass100gone))
# with open("CAS_concentration_massLE1002021.pkl", "wb") as f:
#     pickle.dump(total_concentration_cm3_mass100gone, f)
# print("Saved CAS concentration filtered dataset.")
...
print("After matching slope/mass legs:", len(filtered_master_BCB_ddry_mass100gone))
# with open("CAS_ddry_massLE1002021.pkl", "wb") as f:
...
# np.savez("CAS_average_drysize_nomass1002021.npz",
#          bins=common_bins,
#          average=average_dN_dD_dry)
# print("Saved CAS averaged distribution.")
# print("Saved to:", os.path.abspath("CAS_ddry_massLE1002021.pkl"))
# print("Exists?", os.path.exists("CAS_ddry_massLE1002021.pkl"))
#%%
...
df_wind = df_wind[df_wind["Month"].isin([11, 12])].copy()
...
plt.title("Corrected Wind Speed Timeline (November-December 2021)\nLegs ordered by Date then BCB_start",
          fontsize=14, fontweight="bold")
plt.tight_layout()
plt.show()
#%%
#color coded by month with
month_name = {11:"November", 12:"December"}
plt.figure(figsize=(12, 4.8))
...
plt.title("BCB Wind Speed\nNovember-December 2021 Monthly Means",
...
dfp = dfp[dfp["Month"].isin([11, 12])].copy()
...
ax.set_title("BCB Corrected Wind Speed November-December 2021\nMonthly Trend",
             fontsize=18, fontweight="bold")
...
ax.set_title("Wind Speed November-December 2021\nMonthly Trend",
...
for i, lab in enumerate(labels):
    ...
    lab.set_text("\n" * (i % 4) + base)
ax.set_xticklabels([lab.get_text() for lab in labels])
target_dates = {"2021-11-30": 0, "2021-12-07": 3}
labels = ax.get_xticklabels()
for lab in labels:
...
plt.ylabel("Total Wind Speed Bin \nConcentration (cm$^{-3}$)", fontsize=20, fontweight='bold')
plt.title("CAS Below Cloud Base \n November-December 2021", fontsize=20, fontweight='bold')
plt.legend(fontsize=13, title_fontsize=14, loc='upper right', frameon=False)
plt.tight_layout()
plt.xlim(0, 12)
plt.ylim(-0.2, 1.9)
plt.xticks(fontsize=18, fontweight='bold')
...
save_dir = "/home/disk/eos4/kathem24/activate/data/2021/CAS"
os.makedirs(save_dir, exist_ok=True)
save_path = os.path.join(save_dir, "CAS_ND2021_wind_bin_concentration_plot_data.pkl")
plot_data = {
...
#%%
# import pickle
# load_path = "/home/disk/eos4/kathem24/activate/data/2021/CAS/CAS_ND2021_wind_bin_concentration_plot_data.pkl"
...
plt.title("CAS Below Cloud Base November-December 2021", fontsize=16, fontweight='bold')
legend_labels_mass = [
    plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=colors[idx], markersize=10, 
...
plt.title("CAS Below Cloud Base November-December 2021", fontsize=20, fontweight='bold')
plt.legend(fontsize=13, title_fontsize=14, loc='best', frameon=True)
plt.tight_layout()
plt.xticks(fontsize=20, fontweight='bold')
plt.yticks(fontsize=20, fontweight='bold')
...
save_dir = "/home/disk/eos4/kathem24/activate/data/2021/CAS"
os.makedirs(save_dir, exist_ok=True)
save_path = os.path.join(save_dir, "CAS_ND2021_wind_bin_mass_plot_data.pkl")
mass_plot_data = {
    "windspeed_bins": windspeed_bins,
...
print(f"Saved CAS mass plot data to: {save_path}")
# %%
# import pickle

# load_path = "/home/disk/eos4/kathem24/activate/data/2021/CAS/CAS_ND2021_wind_bin_mass_plot_data.pkl"
