...
for date in dates_legs:
    ...
    leg_dictionary = {
        'Date': date,
        'LegIndex_05': {'StartTimes': [], 'StopTimes': []},
    }
...
# print(f"Before filtering:\n{df_legs[['  LegIndex', 'Time_Start']]}")
        leg_index_05 = df_legs[df_legs['  LegIndex'] % 100 == 5]
        
        leg_dictionary['LegIndex_05']['StartTimes'].extend(leg_index_05['Time_Start'].tolist())
        leg_dictionary['LegIndex_05']['StopTimes'].extend(leg_index_05['  Time_Stop'].tolist())
    #print(leg_data)
...
for date in dates_CAS:
    ...
    ACB_Start = leg_dictionary['LegIndex_05']['StartTimes']
    ACB_Stop = leg_dictionary['LegIndex_05']['StopTimes']
...
ax1.scatter(bin_center,  final_clear_conc[3], label='Leg 4', color='brown')
ax2.scatter(bin_center, final_cloud_conc[3], label='Leg 4', color='brown')
ax1.scatter(bin_center,  final_clear_conc[4], label='Leg 5', color='black')
ax2.scatter(bin_center, final_cloud_conc[4], label='Leg 5', color='black')
ax1.scatter(bin_center,  final_clear_conc[5], label='Leg 6', color='slateblue')
ax2.scatter(bin_center, final_cloud_conc[5], label='Leg 6', color='slateblue')
ax1.scatter(bin_center,  final_clear_conc[6], label='Leg 7', color='darkgreen')
ax2.scatter(bin_center, final_cloud_conc[6], label='Leg 7', color='darkgreen')

ax1.set_xlim([10**0.1, 10**1.8])
ax1.set_ylim([10**-4.5, 10**0])
# ax2.set_xlim([10**0.1, 10**1.8])
# ax2.set_ylim([10**-1, 10**1.5])

# Set plot properties for the first subplot
ax1.set_yscale('log')
ax1.set_xscale('log')
ax1.set_title(f'{date} ACT Clear Mean Droplet\n Concentration vs Bin Size', fontsize=16, fontweight='bold')
