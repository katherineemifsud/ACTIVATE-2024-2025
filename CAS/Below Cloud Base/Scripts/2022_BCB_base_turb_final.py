...
import sys
#%%
#turbulence base 385 g m^-2 LWP 50 eps
...
def inspect_pickle(fname):
    ...
    return data
n0_data_turb = inspect_pickle("n0_r (1).pkl")
R_data_turb  = inspect_pickle("R_eps50.pkl")
r_dry_turb = n0_data_turb[0]
n0_r_turb  = n0_data_turb[1]
extra = n0_data_turb[2] if len(n0_data_turb) > 2 else None
print("\nExtracted:")
print("  r_dry:", np.shape(r_dry_turb))
print("  n0_r :", np.shape(n0_r_turb))
print("  extra data:", type(extra), "\n")
time = R_data_turb[0]
rain_t = R_data_turb[1]
extra_R = R_data_turb[2] if len(R_data_turb) > 2 else None
print("  time  :", np.shape(time))
print("  rain_t:", np.shape(rain_t))
print("  extra R data:", type(extra_R))
#%%
with open(os.path.join(base, "R_eps50.pkl"), "rb") as f:
...
mask = r_dry_turb > 1e-6
gccn_m3_base_turb = np.sum(n0_r_turb[:, mask], axis=1)
for i, (tot, gccn) in enumerate(zip(total_m3, gccn_m3_base_turb), start=1):
    frac = gccn / tot
...
gccn_m3_base_turb = np.sum(n0_r_turb[:, mask], axis=1)
accum_rain_base_turb = np.mean(R_data, axis=1)
accum_rain_base_full_turb = np.array(accum_rain_base_turb, dtype=float)
gccn_m3_base_full_turb = np.array(gccn_m3_base_turb, dtype=float)
for i, (gccn, rain) in enumerate(zip(gccn_m3_base_turb, accum_rain_base_turb), start=1):
    print(f"Leg {i:02d}: GCCN={gccn:.3e} m^-3, Rain={rain:.3f} mm")
plt.figure(figsize=(6, 4.5))
colors = plt.cm.viridis(np.linspace(0, 1, len(gccn_m3_base_turb)))
for i, (gccn, rain, c) in enumerate(zip(gccn_m3_base_turb, accum_rain_base_turb, colors), start=1):
    plt.scatter(gccn, rain, s=80, edgecolor='k', color=c)
valid = (gccn_m3_base_turb > 0) & (accum_rain_base_turb > 0)
logx = np.log10(gccn_m3_base_turb[valid])
logy = np.log10(accum_rain_base_turb[valid])
slope, intercept, r_value, p_value, std_err = linregress(logx, logy)
x_sorted = np.sort(gccn_m3_base_turb[valid])
...
print("Rain full len:", len(accum_rain_base_full_turb))
print("GCCN full len:", len(gccn_m3_base_full_turb))
...
rain_full = np.array(accum_rain_base_full_turb, dtype=float)
gccn_full = np.array(gccn_m3_base_full_turb, dtype=float)
...
accum_rain_base_turb  = rain_full[keep]
gccn_m3_base_turb= gccn_full[keep]
print("Kept legs:", keep.sum())
print("Dropped legs:", (~keep).sum())
print("Now lengths -> mass:", len(all_mass_values), "rain:", len(accum_rain_base_turb), "gccn:", len(gccn_m3_base))
