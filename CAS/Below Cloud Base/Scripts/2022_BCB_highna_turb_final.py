...
R_data  = inspect_pickle("R_hiNa_eps50.pkl")
...
#%%
with open(os.path.join(base, "R_hiNa_eps50.pkl"), "rb") as f:
...
gccn_m3 = np.sum(n0_r_highnaturb[:, mask], axis=1)
R_data = np.asarray(R_data)
accum_rain_highnaturb = np.mean(R_data, axis=1)
accum_rain_highnaturb_full = np.array(accum_rain_highnaturb, dtype=float)
gccn_m3_highnaturb_full = np.array(gccn_m3, dtype=float)
