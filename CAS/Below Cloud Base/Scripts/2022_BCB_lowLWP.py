...
R_data  = inspect_pickle("R_loLWP.pkl")
...
#%%
with open(os.path.join(base, "R_loLWP.pkl"), "rb") as f:
...
gccn_m3 = np.sum(n0_r_lowLWP[:, mask], axis=1)
R_data = np.asarray(R_data)
accum_rain_lowLWP = np.mean(R_data, axis=1)
gccn_m3_lowLWP_full = np.array(gccn_m3, dtype=float)
accum_rain_lowLWP_full = np.array(accum_rain_lowLWP, dtype=float)
