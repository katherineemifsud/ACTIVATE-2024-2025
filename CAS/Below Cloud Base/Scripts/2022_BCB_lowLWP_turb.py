...
R_data  = inspect_pickle("R_loLWP_eps50.pkl")
...
#%%
with open(os.path.join(base, "R_loLWP_eps50.pkl"), "rb") as f:
...
gccn_m3 = np.sum(n0_r_lowLWPturb[:, mask], axis=1)
R_data = np.asarray(R_data)
accum_rain_lowLWPturb = np.mean(R_data, axis=1)
gccn_m3_lowLWPturb_full = np.array(gccn_m3, dtype=float)
accum_rain_lowLWPturb_full = np.array(accum_rain_lowLWPturb, dtype=float)
