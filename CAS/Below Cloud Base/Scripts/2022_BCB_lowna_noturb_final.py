...
import sys
#%%
#no turbulence low 385 g m^-2 LWP low na
...
def inspect_pickle(fname):
    ...
    return data
n0_data_lowna = inspect_pickle("n0_r_lowNa (1).pkl")
R_data_lowna  = inspect_pickle("R_loNa.pkl")
r_dry_lowna = n0_data_lowna[0]
n0_r_lowna  = n0_data_lowna[1]
extra = n0_data_lowna[2] if len(n0_data_lowna) > 2 else None
print("\nExtracted:")
print("  r_dry:", np.shape(r_dry_lowna))
print("  n0_r :", np.shape(n0_r_lowna))
print("  extra data:", type(extra), "\n")
time = R_data_lowna[0]
rain_t = R_data_lowna[1]
extra_R = R_data_lowna[2] if len(R_data_lowna) > 2 else None
print("  time  :", np.shape(time))
print("  rain_t:", np.shape(rain_t))
print("  extra R data:", type(extra_R))
#%%
with open(os.path.join(base, "R_loNa.pkl"), "rb") as f:
...
mask = r_dry_lowna > 1e-6 
gccn_m3_lowna = np.sum(n0_r_lowna[:, mask], axis=1)
R_data_lowna = np.asarray(R_data_lowna)
accum_rain_lowna = np.mean(R_data_lowna, axis=1)
accum_rain_lowna_full = np.array(accum_rain_lowna, dtype=float)
gccn_m3_lowna_full = np.array(gccn_m3_lowna, dtype=float)
for i, (gccn, rain) in enumerate(zip(gccn_m3_lowna, accum_rain_lowna), start=1):
    print(f"Leg {i:02d}: GCCN={gccn:.3e} m^-3, Rain={rain:.3f} mm")
plt.figure(figsize=(6, 4.5))
colors = plt.cm.viridis(np.linspace(0, 1, len(gccn_m3_lowna)))
for gccn, rain, c in zip(gccn_m3_lowna, accum_rain_lowna, colors):
    plt.scatter(gccn, rain, s=80, edgecolor='k', color=c)
valid = (gccn_m3_lowna > 0) & (accum_rain_lowna > 0)
logx = np.log10(gccn_m3_lowna[valid])
logy = np.log10(accum_rain_lowna[valid])
slope, intercept, r_value, p_value, std_err = linregress(logx, logy)
R = r_value
R2 = R**2
print(f"Correlation R = {R:.4f}")
print(f"Coefficient of Determination R² = {R2:.4f}")
x_sorted = np.sort(gccn_m3_lowna[valid])
...
accum_rain_lowna = rain_full[keep]
gccn_m3_lowna = gccn_full[keep]
print("Kept legs:", keep.sum())
print("Dropped legs:", (~keep).sum())
print("Now lengths -> mass:", len(all_mass_values), "rain:", len(accum_rain_lowna), "gccn:", len(gccn_m3_lowna))
