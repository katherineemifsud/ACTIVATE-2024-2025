...
gccn_cases_turb = {
    "Base": {
    "gccn": gccn_m3_base_turb,
    "rain": accum_rain_base_turb,
        "color": "tab:blue",
        "marker": "o"
    },
    "Low Na": {
...
gccn_baseturb,   rain_baseturb   = apply_global_filters_gccn(mass_full, accum_rain_base_full_turb,   gccn_m3_base_full_turb,   good_base)
...
#%%
# 2-panel paper figure for mass vs rain and gccn vs rain, no turbulence vs turbulence
...
fig.suptitle(
    "Precipitation as a function of mass and concentration\nLog-binned median curves",
    fontsize=15,
    fontweight="bold",
    y=0.98
)
...
#     "\nGCCN cases:")
# print(
#     fig5["gccn_cases"].keys())
# print(
#     "\nGCCN turbulence cases:")
# print(
#     fig5["gccn_cases_turb"].keys())
# print(
#     "\nMass cases:")
# print(
#     fig5["mass_cases"].keys())
# print(
#     "\nMass turbulence cases:")
# print(
#     fig5["mass_cases_turb"].keys())
#%%
#comparing with jensen and lee 

#%%
# # Build GCCN/mass/rain/slope cases with slope matched to each plotted point
# def apply_global_filters_all(mass_full, slope_full, rain_full, gccn_full, good_mask):
#     mass_full  = np.asarray(mass_full, dtype=float).ravel()
#     slope_full = np.asarray(slope_full, dtype=float).ravel()
#     rain_full  = np.asarray(rain_full, dtype=float).ravel()
#     gccn_full  = np.asarray(gccn_full, dtype=float).ravel()
#     good_mask  = np.asarray(good_mask, dtype=bool).ravel()
#     assert len(mass_full) == len(slope_full) == len(rain_full) == len(gccn_full) == len(good_mask), \
#         f"Length mismatch: mass={len(mass_full)} slope={len(slope_full)} rain={len(rain_full)} gccn={len(gccn_full)} good={len(good_mask)}"
#     ok = (
#         good_mask &
#         np.isfinite(mass_full)  & (mass_full > 0) &
#         np.isfinite(slope_full) &
#         np.isfinite(rain_full)  & (rain_full > 0) &
#         np.isfinite(gccn_full)  & (gccn_full > 0)
#     )

#     return mass_full[ok], slope_full[ok], rain_full[ok], gccn_full[ok]
# slope_full = np.array([entry["Dry Slope (D)"] for entry in all_mass_sorted], dtype=float)
# mass_base, slope_base, rain_base, gccn_base = apply_global_filters_all(
#     mass_full, slope_full, accum_rain_base_full, gccn_m3_base_full, good_base
# )
# mass_lowna, slope_lowna, rain_lowna, gccn_lowna = apply_global_filters_all(
#     mass_full, slope_full, accum_rain_lowna_full, gccn_m3_lowna_full, good_base
# )
# mass_highna, slope_highna, rain_highna, gccn_highna = apply_global_filters_all(
#     mass_full, slope_full, accum_rain_highna_full, gccn_m3_highna_full, good_base
# )
# mass_lowLWP, slope_lowLWP, rain_lowLWP, gccn_lowLWP = apply_global_filters_all(
#     mass_full, slope_full, accum_rain_lowLWP_full, gccn_m3_lowLWP_full, good_base
# )
# mass_baseturb, slope_baseturb, rain_baseturb, gccn_baseturb = apply_global_filters_all(
#     mass_full, slope_full, accum_rain_base_full_turb, gccn_m3_base_full_turb, good_base
# )
# mass_lownaturb, slope_lownaturb, rain_lownaturb, gccn_lownaturb = apply_global_filters_all(
#     mass_full, slope_full, accum_rain_lownaturb_full, gccn_m3_lownaturb_full, good_base
# )
# mass_highnaturb, slope_highnaturb, rain_highnaturb, gccn_highnaturb = apply_global_filters_all(
#     mass_full, slope_full, accum_rain_highnaturb_full, gccn_m3_highnaturb_full, good_base
# )
# mass_lowLWPturb, slope_lowLWPturb, rain_lowLWPturb, gccn_lowLWPturb = apply_global_filters_all(
#     mass_full, slope_full, accum_rain_lowLWPturb_full, gccn_m3_lowLWPturb_full, good_base
# )
# cb_colors = {
#     "Base": "#0072B2",
#     "Low Na": "#009E73",
...
# }
# gccn_cases = {
#     "Base":               {"gccn": gccn_base,   "rain": rain_base,   "slope": slope_base,   "color": cb_colors["Base"]},
#     "Low Na":             {"gccn": gccn_lowna,  "rain": rain_lowna,  "slope": slope_lowna,  "color": cb_colors["Low Na"]},
#     "High Na":            {"gccn": gccn_highna, "rain": rain_highna, "slope": slope_highna, "color": cb_colors["High Na"]},
#     "100 g m$^{-2}$ LWP": {"gccn": gccn_lowLWP, "rain": rain_lowLWP, "slope": slope_lowLWP, "color": cb_colors["100 g m$^{-2}$ LWP"]},
# }
# gccn_cases_turb = {
#     "Base + Turb":               {"gccn": gccn_baseturb,   "rain": rain_baseturb,   "slope": slope_baseturb,   "color": cb_colors["Base"]},
#     "Low Na + Turb":             {"gccn": gccn_lownaturb,  "rain": rain_lownaturb,  "slope": slope_lownaturb,  "color": cb_colors["Low Na"]},
#     "High Na + Turb":            {"gccn": gccn_highnaturb, "rain": rain_highnaturb, "slope": slope_highnaturb, "color": cb_colors["High Na"]},
#     "100 g m$^{-2}$ LWP + Turb": {"gccn": gccn_lowLWPturb, "rain": rain_lowLWPturb, "slope": slope_lowLWPturb, "color": cb_colors["100 g m$^{-2}$ LWP"]},
# }
# mass_cases = {
#     "Base":               {"mass": mass_base,   "rain": rain_base,   "slope": slope_base,   "color": cb_colors["Base"]},
#     "Low Na":             {"mass": mass_lowna,  "rain": rain_lowna,  "slope": slope_lowna,  "color": cb_colors["Low Na"]},
#     "High Na":            {"mass": mass_highna, "rain": rain_highna, "slope": slope_highna, "color": cb_colors["High Na"]},
#     "100 g m$^{-2}$ LWP": {"mass": mass_lowLWP, "rain": rain_lowLWP, "slope": slope_lowLWP, "color": cb_colors["100 g m$^{-2}$ LWP"]},
# }
# mass_cases_turb = {
#     "Base + Turb":               {"mass": mass_baseturb,   "rain": rain_baseturb,   "slope": slope_baseturb,   "color": cb_colors["Base"]},
#     "Low Na + Turb":             {"mass": mass_lownaturb,  "rain": rain_lownaturb,  "slope": slope_lownaturb,  "color": cb_colors["Low Na"]},
#     "High Na + Turb":            {"mass": mass_highnaturb, "rain": rain_highnaturb, "slope": slope_highnaturb, "color": cb_colors["High Na"]},
#     "100 g m$^{-2}$ LWP + Turb": {"mass": mass_lowLWPturb, "rain": rain_lowLWPturb, "slope": slope_lowLWPturb, "color": cb_colors["100 g m$^{-2}$ LWP"]},
# }
# print("Base no turb:", len(gccn_cases["Base"]["gccn"]), len(gccn_cases["Base"]["rain"]), len(gccn_cases["Base"]["slope"]))
# print("Base turb:", len(gccn_cases_turb["Base + Turb"]["gccn"]), len(gccn_cases_turb["Base + Turb"]["rain"]), len(gccn_cases_turb["Base + Turb"]["slope"]))
# # %%
# def split_turb_label(label):
#     if label.startswith("High Turbulence "):
#         return label.replace("High Turbulence ", ""), "High Turbulence"
...
# g_base = np.asarray(gccn_cases["Base"]["gccn"], dtype=float).ravel()
# r_base = np.asarray(gccn_cases["Base"]["rain"], dtype=float).ravel()
# s_base = np.asarray(gccn_cases["Base"]["slope"], dtype=float).ravel()
# msk_base = (
#     np.isfinite(g_base) &
#     np.isfinite(r_base) &
#     np.isfinite(s_base) &
...
#     (g_base <= xlim1[1])
# )
# g_base_plot, r_base_plot, s_base_plot = sort_by_slope(
#     g_base[msk_base],
#     r_base[msk_base],
#     s_base[msk_base]
# )
...
# m_base = np.asarray(mass_cases["Base"]["mass"], dtype=float).ravel()
# r_base2 = np.asarray(mass_cases["Base"]["rain"], dtype=float).ravel()
# s_base2 = np.asarray(mass_cases["Base"]["slope"], dtype=float).ravel()
# msk_base_mass = (
#     np.isfinite(m_base) &
#     np.isfinite(r_base2) &
#     np.isfinite(s_base2) &
...
#     (m_base <= xlim2[1])
# )
# m_base_plot, r_base2_plot, s_base2_plot = sort_by_slope(
#     m_base[msk_base_mass],
#     r_base2[msk_base_mass],
#     s_base2[msk_base_mass]
# )
# sc2 = ax2.scatter(
#     m_base_plot,
#     r_base2_plot,
...
#     if "(No Turbulence)" in lab:
#         no_h.append(h)
#         no_l.append(lab.replace(" (No Turbulence)", ""))
#     elif "(Turbulence)" in lab:
#         tb_h.append(h)
#         tb_l.append(lab.replace(" (Turbulence)", ""))
# leg1 = fig.legend(
#     no_h,
#     no_l,
#     title=r"$\mathbf{\epsilon = 0\ cm^2\ s^{-3}}$",
#     loc="center left",
#     bbox_to_anchor=(0.93, 0.60),
#     fontsize=14,
#     frameon=False,
# )
# leg1.get_title().set_fontweight("bold")
...
# cbar.set_label("Dry Slope D (µm)", fontsize=14, fontweight="bold")
...
# plt.tight_layout(rect=[0, 0, 0.82, 1])
# plt.show()
# # fig.savefig("model_base_slope_scatter.pdf", dpi=300, bbox_inches="tight")
# # %%
