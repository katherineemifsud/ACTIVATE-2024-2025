...
for i, (gccn, rain, c) in enumerate(zip(gccn_m3, total_rain_m, colors), start=1):
    plt.scatter(gccn, rain, s=80, edgecolor='k', color=c, label=f"Leg {i}")
logx = np.log10(gccn_m3)
logy = np.log10(total_rain_m)
...
plt.ylabel("Total Rainfall (m)", fontsize=16, fontweight="bold")
plt.title("Below Cloud Base\n Feb 15, 2022\nGCCN vs Total Rainfall", fontsize=16, fontweight="bold")
