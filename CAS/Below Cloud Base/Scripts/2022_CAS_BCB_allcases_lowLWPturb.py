...
base = "/home/disk/eos4/kathem24/activate/data/CAS/Full low LWP turb"
...
def inspect_pickle(fname):
    ...
    return data
n0_data = inspect_pickle("n0_r (2).pkl")
R_data  = inspect_pickle("R_lowLWPTurb.pkl")
r_dry_lowLWPturb = n0_data[0]
n0_r_lowLWPturb  = n0_data[1]
extra = n0_data[2] if len(n0_data) > 2 else None
print("\nExtracted:")
print("  r_dry:", np.shape(r_dry_lowLWPturb))
print("  n0_r :", np.shape(n0_r_lowLWPturb))
print("  extra data:", type(extra), "\n")
time_lowLWPturb = R_data[0]
rain_t_lowLWPturb = R_data[1]
extra_R = R_data[2] if len(R_data) > 2 else None
print("  time  :", np.shape(time_lowLWPturb))
print("  rain_t:", np.shape(rain_t_lowLWPturb))
print("  extra R data:", type(extra_R))
#%%
LWP_lowLWPturb = rain_t_lowLWPturb   # because rain_t_lownaturb is actually LWP(t)
precip_accum = np.max(LWP_lowLWPturb, axis=1)[:, None] - LWP_lowLWPturb # units: kg m^-2 = mm 
precip_masked = np.where(time_lowLWPturb >= 800, precip_accum, np.nan)
plt.figure(figsize=(8, 5))
for i in range(precip_masked.shape[0]):
    plt.plot(time_lowLWPturb, precip_masked[i], lw=1.5, alpha=0.85)
...
gccn_m3_lowLWPturb_full = np.array(gccn_m3, dtype=float) # units: kg m^-2 = mm # units: kg m^-2 = mm
for i, (gccn, rain) in enumerate(zip(gccn_m3, accum_rain_lowLWPturb), start=1):
    print(f"Leg {i:02d}: GCCN={gccn:.3e} m^-3, Rain={rain:.3f} mm")
plt.figure(figsize=(6, 4.5))
colors = plt.cm.viridis(np.linspace(0, 1, len(gccn_m3)))
for i, (gccn, rain, c) in enumerate(zip(gccn_m3, accum_rain_lowLWPturb, colors), start=1):
    plt.scatter(gccn, rain, s=80, edgecolor='k', color=c, label=f"Leg {i}")
logx = np.log10(gccn_m3)
logy = np.log10(accum_rain_lowLWPturb)
...
print("Rain full len:", len(accum_rain_lowLWPturb_full))
print("GCCN full len:", len(gccn_m3_lowLWPturb_full))
all_mass = df_mass.to_dict(orient="records")
all_mass_sorted = sorted(all_mass, key=lambda x: (x["Date"], x["BCB_start"]))
all_mass_values = [entry["Dry Mass (µg/m³)"] for entry in all_mass_sorted]
mass_thr = 100  # µg/m³
mass_full = np.array(all_mass_values, dtype=float)
rain_full = np.array(accum_rain_lowLWPturb_full, dtype=float)
gccn_full = np.array(gccn_m3_lowLWPturb_full, dtype=float)
...
accum_rain_lowLWPturb  = rain_full[keep]
gccn_m3_lowLWPturb          = gccn_full[keep]
print("Kept legs:", keep.sum())
print("Dropped legs:", (~keep).sum())
print("Now lengths -> mass:", len(all_mass_values), "rain:", len(accum_rain_lowLWPturb), "gccn:", len(gccn_m3_lowLWPturb))
#%%
mass = np.array(all_mass_values)
rain_lowLWPturb = accum_rain_lowLWPturb    
for i, (m, r) in enumerate(zip(mass, rain_lowLWPturb), start=1):
    print(f"Leg {i:02d}: Mass={m:.2f} µg/m³, Rain={r:.3f} mm")
plt.figure(figsize=(6, 4.5))
colors = plt.cm.plasma(np.linspace(0, 1, len(mass)))

for i, (m, r, c) in enumerate(zip(mass, rain_lowLWPturb, colors), start=1):
    plt.scatter(m, r, s=80, edgecolor='k', color=c, label=f"Leg {i}")
logx = np.log10(mass)
logy = np.log10(rain_lowLWPturb)
slope, intercept, r_value, p_value, std_err = linregress(logx, logy)
R = r_value
R2 = R**2
print(f"Correlation R = {R:.4f}")
...
rain_filt = rain_lowLWPturb[mask_range]
...
for i, (m, g, c) in enumerate(zip(mass, gccn_m3_lowLWPturb, colors), start=1):
...
log_mass = np.log10(mass)
log_gccn = np.log10(gccn_m3_lowLWPturb)
...
for i, (D, g, c) in enumerate(zip(slope_D, gccn_m3_lowLWPturb, colors), start=1):
...
# plt.show()
#%%
all_entries_lowLWPturb = [entry for entry, k in zip(all_mass_sorted, keep) if k]
dfp = pd.DataFrame(all_entries_lowLWPturb).copy()
...
x = np.arange(len(dfp))
gccn_arr = np.asarray(gccn_m3_lowLWPturb, dtype=float) * 1e-6  # m^-3 -> cm^-3
month_name = {
    1: "January",
    2: "February",
    3: "March",
