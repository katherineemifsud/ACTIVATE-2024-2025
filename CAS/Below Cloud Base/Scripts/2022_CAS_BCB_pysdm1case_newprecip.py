...
trial_case = "/home/disk/eos4/kathem24/activate/data/CAS/one month size distributions/Feb15HighPrecip/0 (1).pickle"
...
#%%
base_path = "/home/disk/eos4/kathem24/activate/data/CAS/one month size distributions/Feb15HighPrecip"
num_legs = 14
all_accum = []  
time_series = [] 

for i in range(num_legs):
    file_path = os.path.join(base_path, f"{i} (1).pickle")
...
for leg_idx, fit in enumerate(fits_feb15):
    model_path = f"/home/disk/eos4/kathem24/activate/data/CAS/one month size distributions/Feb15HighPrecip/{leg_idx} (1).pickle"
...
for i in range(num_legs):
    ...
    dt = np.median(np.diff(time_s))
    accum_precip_mm = np.cumsum(precip_rate * dt)       # cumulative precip (mm)
    total_precip_mm = accum_precip_mm[-1]
    ...
    print(f"Leg {i:02d}: Total accumulation = {total_precip_mm:.3e} mm")
    plt.figure(figsize=(8, 5))
    plt.plot(time_s / 60, precip_rate * 3600, lw=2, color="tab:blue", label="Precipitation Rate")
    plt.xlabel("Time (minutes)", fontsize=14, fontweight="bold")
    plt.ylabel("Precipitation Rate (mm hr$^{-1}$)", fontsize=14, fontweight="bold")
    plt.title(f"Feb 15 2022 – Leg {i+1}\nTotal Accumulated Precipitation = {total_precip_mm:.2e} mm",
              fontsize=13, fontweight="bold")
    plt.yscale("log")
    plt.grid(True, which="both", ls="--", alpha=0.4)
...
for leg_idx, fit in enumerate(fits_feb15):
    ...
    spec_mean = np.nanmean(spec_all, axis=0)     # [#/m⁴]
    cum_model = -trapz_func(spec_mean[::-1], r_[::-1], initial=0)[::-1]
    N_fit_diff = n0 * 1e6 * np.exp(-r_ / (D * 1e-6 / 2))  # [#/m³/m]
    mask_gccn = r_ > 0.5e-6
    N_gccn_m3 = np.trapz(N_fit_diff[mask_gccn], r_[mask_gccn])  # [#/m³]
    N_gccn_cm3 = N_gccn_m3 / 1e6                                # [#/cm³]
    all_gccn.append(N_gccn_cm3)
    precip_rate = np.asarray(trial["surface precipitation"]).squeeze()
    t = np.asarray(trial["t"]).squeeze()
    if len(precip_rate) == 0:
        total_precip_mm = np.nan
    else:
        dt = np.median(np.diff(t))
        total_precip_mm = np.sum(precip_rate * dt)
    all_accum.append(total_precip_mm)

    print(f"Leg {leg_idx+1:02d}: GCCN(>1µm)={N_gccn_cm3:.3e} cm⁻³,  Rain={total_precip_mm:.3e} mm")
    plt.figure(figsize=(7,5))
    ...
    plt.xlim(0, 1e-5)
    plt.xlabel("Dry Radius (m)", fontsize=16, fontweight="bold")
    plt.ylabel("Number (m$^{-3}$ m$^{-1}$)", fontsize=16, fontweight="bold")
    plt.title(f"Feb 15 2022 – Leg {leg_idx+1}", fontsize=17, fontweight="bold")
...
for leg_idx, fit in enumerate(fits_feb15):
    ...
    cum_model = -trapz_func(spec_mean[::-1], r_[::-1], initial=0)[::-1]
    mask_gccn = r_ > 0.5e-6
    N_gccn_m3 = np.trapz(spec_mean[mask_gccn], r_[mask_gccn])
    N_gccn_cm3 = N_gccn_m3 / 1e6
    all_gccn.append(N_gccn_cm3)
    precip_rate = np.asarray(trial["surface precipitation"]).squeeze()
    time_s = np.asarray(trial["t"]).squeeze()
    if precip_rate.size == 0:
        total_precip_mm = np.nan
    else:
        dt = np.median(np.diff(time_s))
        accum_precip_mm = np.cumsum(precip_rate * dt)
    ...
    print(f"Leg {leg_idx+1:02d}: GCCN = {N_gccn_cm3:.3e} cm⁻³, Rain = {total_precip_mm:.3e} mm")
gccn_conc = np.array(all_gccn)
rain_accum = np.array(all_accum)
valid = (gccn_conc > 0) & (rain_accum > 0) & np.isfinite(gccn_conc) & np.isfinite(rain_accum)

df = pd.DataFrame({
    "Leg": np.arange(1, len(all_gccn) + 1)[valid],
    "GCCN": gccn_conc[valid],
    "Rain": rain_accum[valid]
})
logx = np.log10(df["GCCN"])
logy = np.log10(df["Rain"])
slope, intercept, r_value, p_value, std_err = linregress(logx, logy)
...
palette = sns.color_palette("tab20", len(df))
for i, row in df.iterrows():
    plt.scatter(row["GCCN"], row["Rain"], color=palette[int(row["Leg"])-1],
                s=80, edgecolor="k", label=f"Leg {int(row['Leg'])}")
...
plt.xscale("log")
plt.yscale("log")
plt.xlabel("Total GCCN Concentration (>1 µm D) (cm$^{-3}$)", fontsize=17, fontweight="bold")
plt.ylabel("Total Accumulated Rainfall (mm)", fontsize=17, fontweight="bold")
plt.title("Below Cloud Base \nFeb 15, 2022\nRainfall vs GCCN Concentration",
          fontsize=17, fontweight="bold")
plt.xticks(fontsize=16, fontweight="bold")
plt.yticks(fontsize=16, fontweight="bold")
plt.legend(fontsize=10, ncol=3, loc="lower right")
plt.tight_layout()
plt.show()
print(f"Slope = {slope:.3f},  r = {r_value:.3f},  r² = {r2:.3f}")
