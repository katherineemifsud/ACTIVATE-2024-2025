...
for date in dates_CAS:
    ...
    for file_path in fname_CAS:
        ...
        run = run+1 
#%%
#import the slope dataset
with open("CAS_slope_massLE100.pkl", "rb") as f:
...
intercept_values = np.array([
    e["Dry Intercept (N0)"] for e in filtered_slope_mass100gone
])
mass_values = np.array([
    e["Dry Mass (µg/m³)"] for e in filtered_slope_mass100gone
])
#%%
...
slope_flight_totals = defaultdict(
    lambda: {'Legs': [], 'Total_GCCN_Slope': 0, 'Leg_Count': 0}
)
for entry in filtered_slope_mass100gone:
    date = entry['Date']
    start_time = entry['BCB_start']
    stop_time = entry['BCB_stop']
    slope = entry['Dry Slope (D)']
    slope_flight_totals[date]['Legs'].append({
        'Leg_start': start_time,
        'Leg_stop': stop_time,
        'Leg_GCCN_Slope': slope
    })
    slope_flight_totals[date]['Total_GCCN_Slope'] += slope
    slope_flight_totals[date]['Leg_Count'] += 1
slope_flight_totals = dict(slope_flight_totals)
#%%
average_slope_per_flight = {}
for date, flight_data in slope_flight_totals.items():
    if flight_data['Leg_Count'] > 0:
        average_slope_per_flight[date] = (
            flight_data['Total_GCCN_Slope'] / flight_data['Leg_Count']
        )
    else:
        average_slope_per_flight[date] = np.nan
print("Average GCCN Slope per Flight:")
for date, avg_slope in average_slope_per_flight.items():
    print(f"{date}: {avg_slope:.3f}")
#%%
slope_values = np.array(list(average_slope_per_flight.values()))
...
plt.xticks(fontsize=19, fontweight='bold')
plt.yticks(fontsize=19, fontweight='bold')
plt.title('High GCCN Slope Flights (CAS Jan–Jun 2022)', fontsize=19, fontweight='bold')
plt.tight_layout()
plt.show()
plt.figure(figsize=(8, 6))
plt.pcolormesh(
...
plt.xticks(fontsize=19, fontweight='bold')
plt.yticks(fontsize=19, fontweight='bold')
plt.title('Low GCCN Slope Flights (CAS Jan–Jun 2022)', fontsize=19, fontweight='bold')
plt.tight_layout()
plt.show()
#%%
...
plt.xlabel('Nr+Nc /cm³', fontsize=16, fontweight='bold')
plt.ylabel('LWC g/m³', fontsize=16, fontweight='bold')
plt.title('High GCCN Slope CAS Flights\nJanuary-June 2022',
          fontsize=18, fontweight='bold')
plt.tight_layout()
plt.show()
plt.figure(figsize=(8, 6))
...
plt.xlabel('Nr+Nc /cm³', fontsize=16, fontweight='bold')
plt.ylabel('LWC g/m³', fontsize=16, fontweight='bold')
plt.title('Low GCCN Slope CAS Flights\nJanuary-June 2022',
          fontsize=18, fontweight='bold')
plt.tight_layout()
plt.show()
#%%
...
plt.xticks(fontsize=19, fontweight='bold')
plt.yticks(fontsize=19, fontweight='bold')
plt.title('Mean RWC High GCCN Slope CAS Flights \nJanuary–June 2022)', fontsize=19, fontweight='bold')
plt.tight_layout()
plt.show()
plt.figure(figsize=(8, 6))
...
plt.xticks(fontsize=19, fontweight='bold')
plt.yticks(fontsize=19, fontweight='bold')
plt.title('Mean RWC — Low GCCN Slope CAS Flights January–June 2022', fontsize=19, fontweight='bold')
plt.tight_layout()
plt.show()
#%%
#fixing gray NANs
...
plt.xticks(fontsize=19, fontweight='bold')
plt.yticks(fontsize=19, fontweight='bold')
plt.title('Mean LWC — High GCCN Slope CAS Flights January–June 2022', fontsize=19, fontweight='bold')
plt.tight_layout()
plt.show()
...
plt.xticks(fontsize=19, fontweight='bold')
plt.yticks(fontsize=19, fontweight='bold')
plt.title('Mean LWC — Low GCCN Slope CAS Flights January–June 2022', fontsize=19, fontweight='bold')
plt.tight_layout()
plt.show()
#%%
...
plt.xlabel(r"Nr+Nc (cm$^{-3}$)", fontsize=19, fontweight="bold")
plt.ylabel(r"LWC (g m$^{-3}$)", fontsize=19, fontweight="bold")
plt.title("CAS RWC High / Low GCCN SlopeFlights", fontsize=19, fontweight="bold")
plt.tight_layout()
plt.show()
plt.figure(figsize=(8,6))
img = plt.pcolormesh(
...
def plot_histograms_with_percentage_ratio(boot_dists):
    ...
    plt.show()
gccn_values = np.array(list(average_slope_per_flight.values()))
threshold = np.percentile(gccn_values, 50)
high_dates = {date for date, val in average_slope_per_flight.items() if val >= threshold}
low_dates = {date for date, val in average_slope_per_flight.items() if val < threshold}
