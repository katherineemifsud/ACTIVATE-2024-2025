...
def inspect_pickle(fname):
    ...
    return data
n0_data = inspect_pickle("n0_r_lowNa (3).pkl")
R_data  = inspect_pickle("R_lowNaTurb2.pkl")
r_dry_lowna_turb2 = n0_data[0]
n0_r_lowna_turb2  = n0_data[1]
extra = n0_data[2] if len(n0_data) > 2 else None
print("\nExtracted:")
print("  r_dry:", np.shape(r_dry_lowna_turb2))
print("  n0_r :", np.shape(n0_r_lowna_turb2))
...
#%%
LWP_lowna_turb2 = rain_t_lowna   # because rain_t_lowna is actually LWP(t)
precip_accum = np.max(LWP_lowna_turb2, axis=1)[:, None] - LWP_lowna_turb2    
...
total_m3 = np.sum(n0_r_lowna_turb2, axis=1)
mask = r_dry_lowna_turb2 > 1e-6
...
for i, (gccn, rain, c) in enumerate(zip(gccn_m3, accum_rain_lowna_turb2, colors), start=1):
    plt.scatter(gccn, rain, s=80, edgecolor='k', color=c, label=f"Leg {i}")
logx = np.log10(gccn_m3)
logy = np.log10(accum_rain_lowna_turb2)
...
print("Now lengths -> mass:", len(all_mass_values), "rain:", len(accum_rain_lowna_turb2), "gccn:", len(gccn_m3_lowna_turb2))

#%%
mass = np.array(all_mass_values)
rain_lowna = accum_rain_lowna_turb2
...
plt.title("BCB January - June 2022\n 385 g m$^{-2}$ LWP & low Na\nAccumulated Rainfall\High Turbulence", 
          fontweight="bold", fontsize=18)
# plt.legend(bbox_to_anchor=(1.05, 1), loc="upper left", fontsize=8)
plt.grid(alpha=0.3)
...
rain_mm = accum_rain_lowna_turb2
plt.figure(figsize=(6, 4.5))
...
for i, (m, g, c) in enumerate(zip(mass, gccn_m3_lowna_turb2, colors), start=1):
...
log_mass = np.log10(mass)
log_gccn = np.log10(gccn_m3_lowna_turb2)
...
for i, (D, g, c) in enumerate(zip(slope_D, gccn_m3_lowna_turb2, colors), start=1):
...
# plt.show()
#%%
all_entries_lowna_turb2 = [entry for entry, k in zip(all_mass_sorted, keep) if k]
dfp = pd.DataFrame(all_entries_lowna_turb2).copy()
...
x = np.arange(len(dfp))
gccn_arr = np.asarray(gccn_m3_lowna_turb2, dtype=float) * 1e-6  # m^-3 -> cm^-3
month_name = {
    1: "January",
    2: "February",
    3: "March",
...
ax.set_title("No Turbulence 385 g m$^{-2}$ LWP Total GCCN Concentration Low Na\n Less than 100 µg/m³ mass\nHigh Turbulence",
          fontsize=18, fontweight="bold")
