...
for i in range(len(dates_legs)):
    ...
    for j in range(len(BCB_Start)):
        ...
        all_BCB_means.append(rh_times)  # List that contains all the Min wind/alt mean dictionaries for 1 flight

    master_BCB_RH.append(all_BCB_means)  # List that contains all Min flights
...
for flight in master_BCB:
    for wind_alt in flight:
        ...
        date = wind_alt['Date']
        start_time = wind_alt.get('BCB_start', 'Unknown')  # Replace 'Unknown' if not present
        end_time = wind_alt.get('BCB_end', 'Unknown')      # Replace 'Unknown' if not present
