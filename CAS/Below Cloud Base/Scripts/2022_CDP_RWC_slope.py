...
#import slope data
import pickle
with open("CDP_slope_massLE100.pkl", "rb") as f:
    filtered_slope_mass100gone_CDP = pickle.load(f)
slope_values_CDP = np.array([
    e["Dry Slope (D)"] for e in filtered_slope_mass100gone_CDP
])
intercept_values_CDP = np.array([
    e["Dry Intercept (N0)"] for e in filtered_slope_mass100gone_CDP
])
mass_values_CDP = np.array([
    e["Dry Mass (µg/m³)"] for e in filtered_slope_mass100gone_CDP
])
#%%
...
slope_flight_totals_CDP = defaultdict(
    lambda: {'Legs': [], 'Total_GCCN_Slope': 0, 'Leg_Count': 0}
)
for entry in filtered_slope_mass100gone_CDP:
    date = entry['Date']
    start_time = entry['BCB_start']
    stop_time = entry['BCB_stop']
    slope = entry['Dry Slope (D)']
    slope_flight_totals_CDP[date]['Legs'].append({
        'Leg_start': start_time,
        'Leg_stop': stop_time,
        'Leg_GCCN_Slope': slope
    })
    slope_flight_totals_CDP[date]['Total_GCCN_Slope'] += slope
    slope_flight_totals_CDP[date]['Leg_Count'] += 1
slope_flight_totals_CDP = dict(slope_flight_totals_CDP)
#%%
average_slope_per_flight_CDP = {}
for date, flight_data in slope_flight_totals_CDP.items():
    if flight_data['Leg_Count'] > 0:
        average_slope_per_flight_CDP[date] = (
            flight_data['Total_GCCN_Slope'] / flight_data['Leg_Count']
        )
    else:
        average_slope_per_flight_CDP[date] = np.nan
print("Average GCCN Slope per Flight:")
for date, avg_slope in average_slope_per_flight_CDP.items():
    print(f"{date}: {avg_slope:.3f}")
#%%
slope_values = np.array(list(average_slope_per_flight_CDP.values()))
slope_threshold = np.percentile(slope_values, 50)
high_slope_flights = {}
low_slope_flights = {}
for date, avg_slope in average_slope_per_flight_CDP.items():
    if avg_slope >= slope_threshold:
        high_slope_flights[date] = avg_slope
    else:
        low_slope_flights[date] = avg_slope
print(f"\nSlope Threshold: Low < {slope_threshold:.3f}, High ≥ {slope_threshold:.3f}")
#%%
slope_values = np.array(list(average_slope_per_flight_CDP.values()))
high_slope_values = np.array(list(high_slope_flights.values()))
low_slope_values = np.array(list(low_slope_flights.values()))
df_slope = pd.DataFrame({
    "GCCN Slope (D)": np.concatenate([high_slope_values, low_slope_values]),
    "Flight Type": ["High GCCN Slope"] * len(high_slope_values)
                 + ["Low GCCN Slope"] * len(low_slope_values)
})
plt.figure(figsize=(8, 6))
sns.violinplot(
    x="Flight Type",
...
plt.ylabel("GCCN Slope (D)", fontsize=20, fontweight="bold")
plt.xlabel("GCCN Flight Category", fontsize=20, fontweight="bold")
plt.title("Comparison of High & Low GCCN Slope Flight Categories",
          fontsize=16, fontweight="bold")
plt.grid(axis="y", linestyle="--", alpha=0.7)
plt.tick_params(axis="both", which="major",
                labelsize=18, width=3, length=8)
plt.tick_params(axis="both", which="minor",
                labelsize=18, width=2, length=5)
plt.tight_layout()
plt.show()
#%%
avg_high_slope = np.mean(high_slope_values)
avg_low_slope = np.mean(low_slope_values)
num_high_slope_flights = len(high_slope_values)
num_low_slope_flights = len(low_slope_values)
print(f"Average High GCCN Slope Flight: {avg_high_slope:.3f}")
print(f"Number of High GCCN Slope Flights: {num_high_slope_flights}")
print(f"Average Low GCCN Slope Flight: {avg_low_slope:.3f}")
print(f"Number of Low GCCN Slope Flights: {num_low_slope_flights}")
#%%
high_slope_data = [entry for entry in total_combined_concentration if entry['Date'] in high_slope_flights]
low_slope_data = [entry for entry in total_combined_concentration if entry['Date'] in low_slope_flights]
high_concentration = np.array([entry['Total_Combined_Concentration'] for entry in high_slope_data])
high_lwc = np.array([entry['Total_Liquid_Water'] for entry in total_liquid_water if entry['Date'] in high_slope_flights])
high_rwc = np.array([entry['RWC'] for entry in total_liquid_water if entry['Date'] in high_slope_flights])
low_concentration = np.array([entry['Total_Combined_Concentration'] for entry in low_slope_data])
low_lwc = np.array([entry['Total_Liquid_Water'] for entry in total_liquid_water if entry['Date'] in low_slope_flights])
low_rwc = np.array([entry['RWC'] for entry in total_liquid_water if entry['Date'] in low_slope_flights])
num_bins = 5
x_bins = np.logspace(
    np.log10(1),
    np.log10(max(high_concentration.tolist() + low_concentration.tolist())),
    num_bins
)
y_bins = np.logspace(
    np.log10(min(high_lwc.tolist() + low_lwc.tolist())),
    np.log10(max(high_lwc.tolist() + low_lwc.tolist())),
    num_bins
)
sum_rwc_high, xedges, yedges = np.histogram2d(
    high_concentration, high_lwc,
    bins=[x_bins, y_bins],
    weights=high_rwc
)
...
avg_rwc_high = np.divide(
    sum_rwc_high, counts_high,
    out=np.full_like(sum_rwc_high, np.nan),
    where=counts_high > 0
)
avg_lwc_high = np.divide(
    sum_lwc_high, counts_high,
    out=np.full_like(sum_lwc_high, np.nan),
    where=counts_high > 0
)
rwc_lwc_ratio_high = np.divide(
    avg_rwc_high, avg_lwc_high,
    out=np.full_like(avg_rwc_high, np.nan),
    where=avg_lwc_high > 0
) * 100
masked_rwc_high = np.ma.masked_where(
    np.isnan(rwc_lwc_ratio_high),
    rwc_lwc_ratio_high
)
sum_rwc_low, _, _ = np.histogram2d(
    low_concentration, low_lwc,
    bins=[x_bins, y_bins],
    weights=low_rwc
)
...
counts_cas_high_slope = counts_high.copy()
counts_cas_low_slope = counts_low.copy()
avg_rwc_low = np.divide(
    sum_rwc_low, counts_low,
    out=np.full_like(sum_rwc_low, np.nan),
    where=counts_low > 0
)
avg_lwc_low = np.divide(
    sum_lwc_low, counts_low,
    out=np.full_like(sum_lwc_low, np.nan),
    where=counts_low > 0
)
rwc_lwc_ratio_low = np.divide(
    avg_rwc_low, avg_lwc_low,
    out=np.full_like(avg_rwc_low, np.nan),
    where=avg_lwc_low > 0
) * 100
masked_rwc_low = np.ma.masked_where(
    np.isnan(rwc_lwc_ratio_low),
    rwc_lwc_ratio_low
)
plt.figure(figsize=(8, 6))
norm = mcolors.Normalize(vmin=1, vmax=100)
...
plt.xticks(fontsize=19, fontweight='bold')
plt.yticks(fontsize=19, fontweight='bold')
plt.title('High GCCN Slope Flights (CDP Jan–Jun 2022)', fontsize=19, fontweight='bold')
plt.tight_layout()
plt.show()
plt.figure(figsize=(8, 6))
plt.pcolormesh(
...
plt.xticks(fontsize=19, fontweight='bold')
plt.yticks(fontsize=19, fontweight='bold')
plt.title('Low GCCN Slope Flights (CDP Jan–Jun 2022)', fontsize=19, fontweight='bold')
plt.tight_layout()
plt.show()
#%%
...
plt.xlabel('Nr+Nc /cm³', fontsize=16, fontweight='bold')
plt.ylabel('LWC g/m³', fontsize=16, fontweight='bold')
plt.title('High GCCN Slope CDP Flights\nJanuary-June 2022',
          fontsize=18, fontweight='bold')
plt.tight_layout()
plt.show()
plt.figure(figsize=(8, 6))
...
plt.xlabel('Nr+Nc /cm³', fontsize=16, fontweight='bold')
plt.ylabel('LWC g/m³', fontsize=16, fontweight='bold')
plt.title('Low GCCN Slope CDP Flights\nJanuary-June 2022',
          fontsize=18, fontweight='bold')
plt.tight_layout()
plt.show()
#%%
...
plt.xticks(fontsize=19, fontweight='bold')
plt.yticks(fontsize=19, fontweight='bold')
plt.title('Mean RWC High GCCN Slope CDP Flights \nJanuary–June 2022)', fontsize=19, fontweight='bold')
plt.tight_layout()
plt.show()
plt.figure(figsize=(8, 6))
...
plt.xticks(fontsize=19, fontweight='bold')
plt.yticks(fontsize=19, fontweight='bold')
plt.title('Mean RWC — Low GCCN Slope CDP Flights January–June 2022', fontsize=19, fontweight='bold')
plt.tight_layout()
plt.show()
#%%
#fixing gray NANs
...
plt.xticks(fontsize=19, fontweight='bold')
plt.yticks(fontsize=19, fontweight='bold')
plt.title('Mean LWC — High GCCN Slope CDP Flights January–June 2022', fontsize=19, fontweight='bold')
plt.tight_layout()
plt.show()
...
plt.xticks(fontsize=19, fontweight='bold')
plt.yticks(fontsize=19, fontweight='bold')
plt.title('Mean LWC — Low GCCN Slope CDP Flights January–June 2022', fontsize=19, fontweight='bold')
plt.tight_layout()
plt.show()
#%%
...
plt.tick_params(axis='both', which='major', labelsize=19, width=3, length=8)
plt.tick_params(axis='both', which='minor', labelsize=19, width=2, length=5)
plt.title("CDP (in cloud)\nRWC Ratio High / Low GCCN Slope Flights\nJanuary–June 2022", fontsize=19, fontweight="bold")
plt.tight_layout()
plt.show()
#%%
...
plt.xlabel(r"Nr+Nc (cm$^{-3}$)", fontsize=19, fontweight="bold")
plt.ylabel(r"LWC (g m$^{-3}$)", fontsize=19, fontweight="bold")
plt.title("CDP RWC High / Low GCCN SlopeFlights", fontsize=19, fontweight="bold")
plt.tight_layout()
plt.show()
plt.figure(figsize=(8,6))
img = plt.pcolormesh(
...
cbar = plt.colorbar(img)
cbar.set_label("CDP LWC (High / Low)", fontsize=18, fontweight="bold")
cbar.ax.tick_params(labelsize=16)
for t in cbar.ax.get_yticklabels():
    t.set_fontweight("bold")
plt.xscale("log")
...
plt.xlabel(r"Nr+Nc (cm$^{-3}$)", fontsize=19, fontweight="bold")
plt.ylabel(r"LWC (g m$^{-3}$)", fontsize=19, fontweight="bold")
plt.title("LWC High / Low GCCN Slope Flights", fontsize=19, fontweight="bold")
plt.tight_layout()
plt.show()
#%%
custom_bounds = [0, 0.2, 0.5, 1.0, 1.1, 1.2, 1.3, 1.4, 2.0, 2.2, 2.4, 2.7, 3.0, 3.5, 3.6, 7, 8.5]
...
plt.tick_params(axis='both', which='major', labelsize=19, width=3, length=8)
plt.tick_params(axis='both', which='minor', labelsize=19, width=2, length=5)
plt.title("CDP (in cloud)\nRWC Ratio High / Low Slope GCCN Flights\nJanuary–June 2022", fontsize=19, fontweight="bold")
plt.tight_layout()
plt.show()
#%%
# Merge slope and liquid water lists into a unified dataset
combined_dataset = []
...
def plot_histograms_with_percentage_ratio(boot_dists):
    ...
    plt.show()
gccn_values = np.array(list(average_slope_per_flight_CDP.values()))
threshold = np.percentile(gccn_values, 50)
high_dates = {date for date, val in average_slope_per_flight_CDP.items() if val >= threshold}
low_dates = {date for date, val in average_slope_per_flight_CDP.items() if val < threshold}
...
def plot_histograms_with_percentage_ratio(boot_dists):
    ...
    plt.tight_layout(rect=[0, 0, 1, 0.95])  
    plt.show()
high_dates = set(high_slope_flights.keys())
low_dates = set(low_slope_flights.keys())
high_data = [entry for entry in combined_dataset if entry['Date'] in high_dates]
low_data  = [entry for entry in combined_dataset if entry['Date'] in low_dates]
grouped_high = group_by_flight(high_data)
grouped_low  = group_by_flight(low_data)
    ...
    for j in range(len(y_bins) - 1):
        ...
        if (counts_high[j][i] + counts_low[j][i]) >= min_samples:
            ...
            if mean_val < 0.75 or mean_val > 2.5:
                print(f"spicious bin at Nr+Nc bin {i}, LWC bin {j} → μ = {mean_val:.2f}, σ = {std_val:.2f}, {percent_above:.1f}% > 1")
            label = f"{percent_above:.1f}% > 1\nμ={mean_val:.2f}, σ={std_val:.2f}"
