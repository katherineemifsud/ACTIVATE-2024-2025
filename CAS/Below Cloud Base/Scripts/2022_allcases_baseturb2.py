...
def inspect_pickle(fname):
    ...
    return data
n0_data = inspect_pickle("n0_r (4).pkl")
R_data  = inspect_pickle("R_turb2.pkl")
r_dry_turb2 = n0_data[0]
n0_r_turb2  = n0_data[1]
extra = n0_data[2] if len(n0_data) > 2 else None
print("\nExtracted:")
print("  r_dry:", np.shape(r_dry_turb2))
print("  n0_r :", np.shape(n0_r_turb2))
print("  extra data:", type(extra), "\n")
time_turb2 = R_data[0]
rain_t_turb2 = R_data[1]
extra_R = R_data[2] if len(R_data) > 2 else None
print("  time  :", np.shape(time_turb2))
print("  rain_t:", np.shape(rain_t_turb2))
print("  extra R data:", type(extra_R))
#%%
LWP_turb2 = rain_t_turb2   # because rain_t_turb2 is actually LWP(t)
precip_accum = np.max(LWP_turb2, axis=1)[:, None] - LWP_turb2 
precip_masked = np.where(time_turb2 >= 800, precip_accum, np.nan)
plt.figure(figsize=(8, 5))
for i in range(precip_masked.shape[0]):
    plt.plot(time_turb2, precip_masked[i], lw=1.5, alpha=0.85)

plt.xlabel("Time (s)", fontweight="bold", fontsize=16)
plt.ylabel("Accumulated Rain (mm)", fontweight="bold", fontsize=16)
plt.title("BCB January - June 2022\n 385 g m$^{-2}$ LWP & high turbulence \nAccumulated Rainfall", 
...
mask = r_dry_turb2 > 1e-6
gccn_m3 = np.sum(n0_r_turb2[:, mask], axis=1)
...
mask = r_dry_turb2 > 1e-6  # radius > 1 µm → diameter > 2 µm
gccn_m3 = np.sum(n0_r_turb2[:, mask], axis=1)
accum_rainbaseturb2 = np.max(LWP_turb2, axis=1) - LWP_turb2 [:, -1]
gccn_m3_baseturb_full2 = np.array(gccn_m3, dtype=float) # units: kg m^-2 = mm
accum_rain_baseturb_full2 = np.array(accum_rainbaseturb2, dtype=float)  # units: k  # units: kg m^-2 = mm
for i, (gccn, rain) in enumerate(zip(gccn_m3, accum_rainbaseturb2), start=1):
    print(f"Leg {i:02d}: GCCN={gccn:.3e} m^-3, Rain={rain:.3f} mm")
plt.figure(figsize=(6, 4.5))
colors = plt.cm.viridis(np.linspace(0, 1, len(gccn_m3)))
for i, (gccn, rain, c) in enumerate(zip(gccn_m3, accum_rainbaseturb2, colors), start=1):
    plt.scatter(gccn, rain, s=80, edgecolor='k', color=c, label=f"Leg {i}")
logx = np.log10(gccn_m3)
logy = np.log10(accum_rainbaseturb2)
...
print("Rain full len:", len(accum_rain_baseturb_full2))
print("GCCN full len:", len(gccn_m3_baseturb_full2))
all_mass = df_mass.to_dict(orient="records")
all_mass_sorted = sorted(all_mass, key=lambda x: (x["Date"], x["BCB_start"]))
all_mass_values = [entry["Dry Mass (µg/m³)"] for entry in all_mass_sorted]
mass_thr = 100  # µg/m³
mass_full = np.array(all_mass_values, dtype=float)
rain_full = np.array(accum_rain_baseturb_full2, dtype=float)
gccn_full = np.array(gccn_m3_baseturb_full2, dtype=float)
...
print("Now lengths -> mass:", len(all_mass_values), "rain:", len(accum_rain_baseturb2), "gccn:", len(gccn_m3_baseturb2))


#%%
mass = np.array(all_mass_values)
rain_turb2 = accum_rain_baseturb2   
for i, (m, r) in enumerate(zip(mass, rain_turb2), start=1):
    print(f"Leg {i:02d}: Mass={m:.2f} µg/m³, Rain={r:.3f} mm")
plt.figure(figsize=(6, 4.5))
colors = plt.cm.plasma(np.linspace(0, 1, len(mass)))

for i, (m, r, c) in enumerate(zip(mass, rain_turb2, colors), start=1):
    plt.scatter(m, r, s=80, edgecolor='k', color=c, label=f"Leg {i}")
logx = np.log10(mass)
logy = np.log10(rain_turb2)
slope, intercept, r_value, p_value, std_err = linregress(logx, logy)
R = r_value
R2 = R**2
print(f"Correlation R = {R:.4f}")
...
rain_filt = rain_turb2[mask_range]
...
for i, (m, g, c) in enumerate(zip(mass, gccn_m3_baseturb2, colors), start=1):
...
log_mass = np.log10(mass)
log_gccn = np.log10(gccn_m3_baseturb2)
...
assert len(gccn_m3_baseturb2) == len(all_mass_sorted), \
    f"Lengths don't match: model={len(gccn_m3_baseturb2)} vs mass={len(all_mass_sorted)}"
months = np.array([int(e["Date"][5:7]) for e in all_mass_sorted], dtype=int)
x = np.arange(len(all_mass_sorted))
gccn_m3_baseturb2 = np.asarray(gccn_m3_baseturb2, dtype=float)
month_name = {
    1: "January",
    2: "February",
    3: "March",
...
gccn_cm3 = gccn_m3_baseturb2  * 1e-6  # m⁻³ → cm⁻³
...
plt.title("Turbulence 385 g m$^{-2}$ LWP Total GCCN Concentration\n January–June 2022 Monthly Means",
          fontsize=18, fontweight="bold")
...
for i, (D, g, c) in enumerate(zip(slope_D, gccn_m3_baseturb2, colors), start=1):
...
#%%
all_entries_baseturb2 = [entry for entry, k in zip(all_mass_sorted, keep) if k]
dfp = pd.DataFrame(all_entries_baseturb2).copy()
...
x = np.arange(len(dfp))
gccn_arr = np.asarray(gccn_m3_baseturb2, dtype=float) * 1e-6  # m^-3 -> cm^-3
month_name = {
    1: "January",
    2: "February",
    3: "March",
...
ax.set_title("HighTurbulence 385 g m$^{-2}$ LWP Total GCCN Concentration\n Less than 100 µg/m³ mass\nJanuary–June 2022 Monthly Trend",
          fontsize=18, fontweight="bold")
