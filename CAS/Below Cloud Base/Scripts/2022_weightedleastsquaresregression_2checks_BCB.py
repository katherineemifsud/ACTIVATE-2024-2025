...
plt.show()
#%%
#Binning by mean windspeed at 0.5 m/s bins
...
valid_indices = [i for i in range(len(wind_speeds_bcb)) if not np.isnan(wind_speeds_bcb[i]) and not np.isnan(sum_values[i])]
wind_speeds_bcb_filtered = [wind_speeds_bcb[i] for i in valid_indices]
...
bin_width = 0.5
bins = np.arange(0, max(wind_speeds_bcb_filtered) + bin_width, bin_width)
binned_wind_speeds = []
...
for i in range(len(bins) - 1):
    bin_min = bins[i]
    bin_max = bins[i + 1]
    bin_indices = [j for j in range(len(wind_speeds_bcb_filtered)) if bin_min <= wind_speeds_bcb_filtered[j] < bin_max]
...
#weighted least squares regression analysis (WLS)

#high variance = low weight and low variance = high weight 
#model is adjusted so that more weight is given to more reliable points
#this minimizes the weighted sum of squared residuals 
...
intercept_std_err, slope_std_err = results.bse
...
plt.title('Below Cloud Base January - June 2022 \n(Binned 0.5 m/s)', fontsize=12, fontweight='bold')
plt.xlabel('Corrected BCB Wind Speed (m/s)', fontsize=12, fontweight='bold')
plt.ylabel('Mean droplet concentration dN (per cm^3) \nfor clear skies', fontsize=12, fontweight='bold')
plt.legend()
# plt.ylim(10**-1.5, 10**2)
# plt.yscale('log')
...
sorted_indices = np.argsort(wind_speeds_bcb_filtered)
wind_speeds_bcb_sorted = np.array(wind_speeds_bcb_filtered)[sorted_indices]
sum_values_sorted = np.array(sum_values_filtered)[sorted_indices]

# Calculate deciles
num_points = len(wind_speeds_bcb_sorted)
percentiles = np.linspace(0, 100, 11)  # Generate 11 percentiles (0, 10, 20, ..., 100)
decile_boundaries = np.percentile(wind_speeds_bcb_sorted, percentiles)
...
for i in range(len(percentiles) - 1):
    ...
    bin_indices = np.where((wind_speeds_bcb_sorted >= bin_min) & (wind_speeds_bcb_sorted < bin_max))[0]
...
# Calculate deciles
num_points = len(wind_speeds_bcb_sorted)
percentiles = np.linspace(0, 100, 21)  # Generate 11 percentiles (0, 10, 20, ..., 100)
decile_boundaries = np.percentile(wind_speeds_bcb_sorted, percentiles)
...
##Slope: 0.0109 ± 0.0122
##Intercept: 0.5542 ± 0.0746
##More reliable results than 20 bins and 15 bins 
#%%
##WLS regression with 9 bins
...
## Slope: 0.0144 ± 0.0133
##Intercept: 0.5350 ± 0.0827
##Least reliable results 
#%%
##WLS regression with 20 bins
sum_values = [entry['Sum'] for entry in Y_sum_list]
wind_speeds_bcb = corrected_calc_bcb['Corrected_bcb_windspeed']
...
##
##Slope: 0.0131 ± 0.0138
##Intercept: 0.5434 ± 0.0827
#More uncertain than 20 bins
# %%

# %%
from scipy.stats import linregress

#Regular linear regression on bins of 10
...
plt.show()
# %%
## WLS regression based on spread/variance 
...
# Filter out NaN values
valid_indices = [i for i in range(len(windspeeds_bcb)) if not np.isnan(windspeeds_bcb[i]) and not np.isnan(sum_values[i])]
wind_speeds_bcb_filtered = np.array([windspeeds_bcb[i] for i in valid_indices])
sum_values_filtered = np.array([sum_values[i] for i in valid_indices])

# Calculate bin boundaries based on variance
bin_width = np.std(wind_speeds_bcb_filtered)  # Adjust this based on your preference
bins = np.arange(min(wind_speeds_bcb_filtered), max(wind_speeds_bcb_filtered) + bin_width, bin_width)
...
for i in range(len(bins) - 1):
    bin_min = bins[i]
    bin_max = bins[i + 1]
    bin_indices = np.where((wind_speeds_bcb_filtered >= bin_min) & (wind_speeds_bcb_filtered < bin_max))[0]
...
plt.show()
# %%
##WLS regression with 10 bins exponential 
sum_values = [entry['Sum'] for entry in Y_sum_list]
wind_speeds_bcb = corrected_calc_bcb['Corrected_bcb_windspeed']
