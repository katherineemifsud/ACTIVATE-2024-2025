...
#%%
sum_values = [entry['Sum'] for entry in Y_sum_list]
wind_speeds_bcb = corrected_calc_bcb['Corrected_bcb_windspeed']

valid_indices = [i for i in range(len(wind_speeds_bcb)) if not np.isnan(wind_speeds_bcb[i]) and not np.isnan(Y_sum_list[i]['Sum'])]
wind_speeds_bcb_filtered = [wind_speeds_bcb[i] for i in valid_indices]
sum_values_filtered = [Y_sum_list[i]['Sum'] for i in valid_indices]

# Create the scatterplot
plt.scatter(wind_speeds_bcb_filtered, sum_values_filtered)
plt.title('Below Cloud Base January - June 2022', fontsize=12, fontweight='bold')
plt.xlabel('Mean windspeed (m/s)', fontsize=12, fontweight='bold')
plt.ylabel('Total droplet concentration dN (per cm^3) \nfor clear skies', fontsize=12, fontweight='bold')
# plt.yscale('log')
# plt.ylim(10**-2, 10**2)
...
# Both min alt and bcb scatterplot
...
windspeeds_bcb = corrected_calc_bcb['Corrected_bcb_windspeed']

# Filter valid indices for below cloud base
valid_indices_bcb = [i for i in range(len(windspeeds_bcb)) if not np.isnan(windspeeds_bcb[i]) and not np.isnan(Y_sum_list[i]['Sum'])]
windspeeds_bcb_filtered = [windspeeds_bcb[i] for i in valid_indices_bcb]
sum_values_bcb_filtered = [Y_sum_list[i]['Sum'] for i in valid_indices_bcb]

# Filter valid indices for minimum altitude
valid_indices_min = [i for i in range(len(windspeeds_min)) if not np.isnan(windspeeds_min[i]) and not np.isnan(Y_sum_list[i]['Sum'])]
windspeeds_min_filtered = [windspeeds_min[i] for i in valid_indices_min]
sum_values_min_filtered = [Y_sum_list[i]['Sum'] for i in valid_indices_min]

# Create the scatterplot
plt.figure(figsize=(10, 6))

# Plot minimum altitude data
plt.scatter(windspeeds_min_filtered, sum_values_min_filtered, label='Minimum Altitude', alpha=0.7, color='green')

# Plot below cloud base data
plt.scatter(windspeeds_bcb_filtered, sum_values_bcb_filtered, label='Below Cloud Base', alpha=0.7, color='blue')

plt.title('Wind Speed vs Droplet Concentration (January - June 2022)', fontsize=14, fontweight='bold')
plt.xlabel('Mean windspeed (m/s)', fontsize=14, fontweight='bold')
plt.ylabel('Total droplet concentration dN (per cm^3) \n for clear skies', fontsize=14, fontweight='bold')
plt.xlim(0, 17)
# plt.ylim(0, 9)
# plt.ylim(10**-1.7, 10**1.5)
# plt.yscale('log')
plt.legend()
plt.show()

#%%
#Binning by 0.5 m/s mean wind speed
...
# Merge wind speed and sum values from both datasets
merged_windspeeds = list(windspeeds_min) + list(windspeeds_bcb)
merged_sum_values = sum_values + sum_values

# Filter valid indices
valid_indices = [i for i in range(len(merged_windspeeds)) if not np.isnan(merged_windspeeds[i]) and not np.isnan(merged_sum_values[i])]
merged_windspeeds_filtered = [merged_windspeeds[i] for i in valid_indices]
merged_sum_values_filtered = [merged_sum_values[i] for i in valid_indices]

# Define binning process
bin_width = 0.5
bins = np.arange(0, max(merged_windspeeds_filtered) + bin_width, bin_width)

# Binning process to calculate mean droplet concentration
binned_wind_speeds = []
...
for i in range(len(bins) - 1):
    bin_min = bins[i]
    bin_max = bins[i + 1]
    bin_indices = [j for j in range(len(merged_windspeeds_filtered)) if bin_min <= merged_windspeeds_filtered[j] < bin_max]
    bin_concentrations = [merged_sum_values_filtered[j] for j in bin_indices]
...
plt.figure(figsize=(10, 6))
plt.scatter(binned_wind_speeds, binned_mean_concentrations, color='black', label='Mean Droplet Conc.')
plt.title('Wind Speed vs Mean Droplet Concentration (January - June 2022) \n(Binned at 0.5 m/s)', fontsize=14, fontweight='bold')
plt.xlabel('Mean windspeed (m/s)', fontsize=12, fontweight='bold')
...
#%%
#WLS for 0.5 m/s binned wind speed 
...
for i in range(len(bins) - 1):
    ...
    if bin_concentrations:
        ...
        binned_wind_speeds.append((bin_min + bin_max) / 2)  # Use the midpoint of the bin for plotting
        binned_mean_concentrations.append(mean_concentration)
        weights.append(len(bin_concentrations))  # Use the number of values as the weight
        print(f"{bin_min}-{bin_max}: {len(bin_concentrations)}")

# Create the new scatterplot of binned means
plt.figure(figsize=(10, 6))
plt.scatter(binned_wind_speeds, binned_mean_concentrations, color='black', label='Binned Mean Droplet Conc.')
plt.title('Wind Speed vs Mean Droplet Concentration (January - June 2022) \n(Binned at 0.5 m/s)', fontsize=14, fontweight='bold')
plt.xlabel('Mean windspeed (m/s)', fontsize=12, fontweight='bold')
plt.ylabel('Mean droplet concentration dN (per cm^3) \nfor clear skies', fontsize=12, fontweight='bold')
plt.xlim(0, 17)
...
plt.figure(figsize=(10, 6))
plt.scatter(binned_wind_speeds, binned_mean_concentrations, color='black', label='Mean Droplet Conc.')
plt.plot(binned_wind_speeds, wls_results.predict(X), color='brown', label='WLS Regression Line')
plt.title('Minimum Altitude and Below Cloud Base \n January - June 2022 (Binned at 0.5 m/s)', fontsize=14, fontweight='bold')
plt.xlabel('Mean windspeed (m/s)', fontsize=12, fontweight='bold')
plt.ylabel('Mean droplet concentration dN (per cm^3) \nfor clear skies', fontsize=12, fontweight='bold')
plt.xlim(0, 17)
# plt.ylim(-1, 3)
...
sorted_indices = np.argsort(merged_windspeeds_filtered)
wind_speeds_sorted = np.array(merged_windspeeds_filtered)[sorted_indices]
sum_values_sorted = np.array(merged_sum_values_filtered)[sorted_indices]

# Calculate bin boundaries based on variance
bin_width = np.std(wind_speeds_sorted)  # Adjust this based on your preference
bins = np.arange(min(wind_speeds_sorted), max(wind_speeds_sorted) + bin_width, bin_width)
...
for i in range(len(bins) - 1):
    bin_min = bins[i]
    bin_max = bins[i + 1]
    bin_indices = np.where((wind_speeds_sorted >= bin_min) & (wind_speeds_sorted < bin_max))[0]
...
# Create the scatterplot of binned means with the linear fit
plt.figure(figsize=(10, 6))
...
plt.title('Minimum Altitude & BCB January - June 2022 \n(Binned by Variance)', fontsize=14, fontweight='bold')
...
# Calculate deciles
num_points = len(wind_speeds_sorted)
percentiles = np.linspace(0, 100, 11)  # Generate 10 percentiles (0, 10, 20, ..., 100)
decile_boundaries = np.percentile(wind_speeds_sorted, percentiles)
...
# Calculate deciles
num_points = len(wind_speeds_sorted)
percentiles = np.linspace(0, 100, 21)  # Generate 10 percentiles (0, 10, 20, ..., 100)
decile_boundaries = np.percentile(wind_speeds_sorted, percentiles)
...
# Calculate deciles
num_points = len(wind_speeds_sorted)
percentiles = np.linspace(0, 100, 16)  # Generate 10 percentiles (0, 10, 20, ..., 100)
decile_boundaries = np.percentile(wind_speeds_sorted, percentiles)
...
##slope: -0.0062 ± 0.0080
##intercept: 0.6306 ± 0.0518

#%%
##20 bins WLS regression
...
##slope: -0.0072 ± 0.0101
##intercept: 0.6359 ± 0.0636
#%%
##15 bins WLS regression
...
##intercept: 0.6469 ± 0.0346

#%%
# WLS for 0.5 m/s with stats output
...
##Slope: -0.0071 ± 0.0113
##Intercept: 0.6360 ± 0.0695
# %%

##WLS regression 0.5 m/s binning exponential 

# Ensure all data points are positive before log transformation
...
# Create the scatter plot with log y-axis
plt.figure(figsize=(10, 6))
plt.scatter(binned_wind_speeds, binned_mean_concentrations, color='black', label='Binned Mean Droplet Conc.')
plt.title('Wind Speed vs Mean Droplet Concentration (January - June 2022) \n(Binned at 0.5 m/s)', fontsize=14, fontweight='bold')
plt.xlabel('Mean windspeed (m/s)', fontsize=12, fontweight='bold')
...
# Plot the regression line in log scale
plt.figure(figsize=(10, 6))
plt.scatter(binned_wind_speeds, binned_mean_concentrations, color='black', label='Mean Droplet Conc.')
...
# %%
## WLS decile regression exponential 
