#%%
#This code is for the linear regression run for May at the minimum altitude level without dust. 
...
plt.title('Minimum altitude w/o dust May 2022', fontsize=14, fontweight='bold')
...
plt.scatter(sum_valuess, corrected_windspeedss)
plt.title('Minimum altitude May w/o dust 2022', fontsize=14, fontweight='bold')
plt.xlabel('Total droplet concentration dN (per cm^3) for clear skies ', fontsize=14, fontweight='bold')
plt.ylabel('Mean windspeed (m/s)', fontsize=14, fontweight='bold')
plt.xscale('log')
...
#Plotting just the regression line with the regression summary

# Filter out NaN values from sum_valuess and corrected_windspeedss
valid_indices = np.logical_not(np.isnan(sum_valuess))
sum_valuess_cleaned = np.array(sum_valuess)[valid_indices]
corrected_windspeedss_cleaned = np.array(corrected_windspeedss)[valid_indices]

# Your existing code for creating the scatterplot
plt.scatter(sum_valuess_cleaned, corrected_windspeedss_cleaned)
plt.title('Minimum altitude May 2022', fontsize=14, fontweight='bold')
plt.xlabel('Total droplet concentration dN (per cm^3) for clear skies ', fontsize=14, fontweight='bold')
plt.ylabel('Mean windspeed (m/s)', fontsize=14, fontweight='bold')
plt.xscale('log')

# Fit the linear regression model for cleaned data
model_Y_cleaned = sm.OLS(corrected_windspeedss_cleaned, sm.add_constant(sum_valuess_cleaned)).fit()

# Extract the coefficients from the cleaned model
intercept_Y_cleaned, slope_Y_cleaned = model_Y_cleaned.params

# Define the range of x values for the regression line
x_range_cleaned = np.linspace(min(sum_valuess_cleaned), max(sum_valuess_cleaned), 100)

# Calculate the corresponding y values using the cleaned regression equation
y_range_cleaned = intercept_Y_cleaned + slope_Y_cleaned * x_range_cleaned

# Plot the regression line for cleaned data
plt.plot(x_range_cleaned, y_range_cleaned, color='red', label='Regression Line')
