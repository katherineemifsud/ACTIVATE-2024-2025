    ...
    for leg in flight:
        ...
        for i in range(len(dates_legs)):
            if dates_legs[i] == date:
                ...
                start_idx = np.where(times_rh == Min_start)[0]
                end_idx = np.where(times_rh == Min_stop)[0]
                ...
                plt.title(f'RH PDF\nDate: {date}, Start: {Min_start}, Stop: {Min_stop}')
                plt.grid(True)
                plt.tight_layout()
                plt.show()
    ...
    for leg in flight:
        ...
        for i in range(len(dates_legs)):
            if dates_legs[i] == date:
                ...
                if len(rh_leg) > 0:
                    all_rh_values_min.extend(rh_leg)

all_rh_values_min = np.array(all_rh_values_min)

plt.figure(figsize=(8, 5))
plt.hist(all_rh_values_min, bins=50, density=True, alpha=0.8, edgecolor='black')
plt.xlabel('Relative Humidity (%)')
plt.ylabel('Probability Density')
plt.title('RH Across all Minimum Altitude \nFlight Legs \n January-June 2022', fontsize=20, fontweight="bold")
plt.grid(True)
plt.tight_layout()
plt.show()
...
all_rh_values_min = np.array(all_rh_values_min)
all_rh_values_min = all_rh_values_min[~np.isnan(all_rh_values_min)]
mean_rh = np.mean(all_rh_values_min)
std_rh = np.std(all_rh_values_min)
percentile_5 = np.percentile(all_rh_values_min, 5)
percentile_95 = np.percentile(all_rh_values_min, 95)
above_100 = np.sum(all_rh_values_min > 100) / len(all_rh_values_min) * 100
below_90 = np.sum(all_rh_values_min < 90) / len(all_rh_values_min) * 100
