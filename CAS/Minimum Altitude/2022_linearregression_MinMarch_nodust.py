#%%
#This code is for the linear regression run for March at the minimum altitude level without dust. 
...
plt.title('Minimum altitude without dust March 2022', fontsize=14, fontweight='bold')
...
plt.title('Minimum altitude March  2022', fontsize=14, fontweight='bold')
...
valid_indices_Y = [i for i in range(len(corrected_windspeeds2)) if not np.isnan(corrected_windspeedss_Y[i]) and not np.isnan(sum_valuess_Y[i])]
...
plt.scatter(sum_valuess, corrected_windspeedss)
plt.title('Minimum altitude March with dust 2022', fontsize=14, fontweight='bold')
plt.xlabel('Total droplet concentration dN (per cm^3) for clear skies ', fontsize=14, fontweight='bold')
plt.ylabel('Mean windspeed (m/s)', fontsize=14, fontweight='bold')
plt.xscale('log')
...
plt.legend()
# %%
indices_to_exclude = [39, 40, 41, 42, 45, 46, 69, 73, 86, 87, 88, 89]

# Exclude the specified indices from Y_sum_list and corrected_windspeeds
Y_min_list_modified = [entry for i, entry in enumerate(Y_min_list) if i not in indices_to_exclude]
corrected_windspeedss_Y_modified = [entry for i, entry in enumerate(corrected_windspeedss) if i not in indices_to_exclude]

# Define the independent variable (mean wind speed) and the dependent variable (droplet concentration)
X_Y_modified = corrected_windspeedss_Y_modified
y_Y_modified = [entry['Sum'] for entry in Y_min_list_modified]
...
# %%
plt.scatter(y_Y_modified, corrected_windspeedss_Y_modified)
plt.title('Minimum altitude March without dust 2022', fontsize=14, fontweight='bold')
plt.xlabel('Total droplet concentration dN (per cm^3) for clear skies ', fontsize=14, fontweight='bold')
plt.ylabel('Mean windspeed (m/s)', fontsize=14, fontweight='bold')
plt.xscale('log')

# Extract the coefficients from the model_Y_modified
intercept_Y_modified, slope_Y_modified = model_Y_modified.params

# Define the range of x values for the regression line
x_range_modified = np.linspace(min(y_Y_modified), max(y_Y_modified), 100)

# Calculate the corresponding y values using the regression equation
y_range_modified = intercept_Y_modified + slope_Y_modified * x_range_modified

# Plot the regression line
plt.plot(x_range_modified, y_range_modified, color='red', label='Regression Line')

# Plotting the regression line with confidence intervals
conf_int_modified = model_Y_modified.conf_int(alpha=0.05)  # 95% confidence intervals
plt.fill_between(x_range_modified, conf_int_modified[0][1] + conf_int_modified[1][1] * x_range_modified, 
                 conf_int_modified[0][0] + conf_int_modified[1][0] * x_range_modified, 
