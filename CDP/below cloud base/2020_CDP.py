...
for i in range(len(dates_legs)):
    ...
    twoDS_flight = twoDS[i]
    rh_flight = h20[i]
    CDP_flight["Time_mid"] = pd.to_numeric(
        CDP_flight["Time_mid"],
        errors="coerce"
    )
    ...
    CDP_times = CDP_flight["Time_mid"].to_numpy()
...
plt.legend()
plt.show()
print(    f"Mean Y-only leg-average LWC: "
    f"{np.mean(lwc_values_Y):.6f} g m^-3")

print(    f"Median Y-only leg-average LWC: "
...
filename = "CDP_surface_area_per_leg_2020.pkl"
