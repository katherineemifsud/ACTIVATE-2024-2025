...
df_cdp = df_cdp[
    df_cdp["Date"].astype(str).str.startswith("2020-")
].copy()
df_cdp["Month"] = (
    df_cdp["Date"]
    .astype(str)
...
dfp_cdp["Date"] = dfp_cdp["Date"].astype(str)
bad_mass_mask = (
    pd.to_numeric(
        dfp_cdp["Dry Mass (µg/m³)"],
        errors="coerce"
    ) < 0.001)
print("Removing from CCDP slope plot:")
print(
    dfp_cdp.loc[
        bad_mass_mask,
        [
...
dfp_cdp = dfp_cdp.loc[~bad_mass_mask].copy()
dfp_cdp["Date_dt"] = pd.to_datetime(dfp_cdp["Date"])
sort_cols = ["Date_dt"]
if "BCB_start" in dfp_cdp.columns:
    sort_cols.append("BCB_start")

dfp_cdp = dfp_cdp.sort_values(
    sort_cols,
    kind="mergesort"
).reset_index(drop=True)
x = np.arange(len(dfp_cdp))
...
date_first = dfp_cdp.groupby(
    dfp_cdp["Date_dt"].dt.date
).head(1)
tick_pos = date_first.index.to_numpy()
tick_lab = date_first["Date_dt"].dt.strftime("%Y-%m-%d").to_numpy()
fig_w = max(18, 0.9 * len(tick_pos))
...
match_cols = ["Date", "BCB_start", "BCB_stop"]
dfp = pd.merge(
    df_mass_slope,
    df_conc[match_cols + ["Total_Y_Concentration_cm3"]],
    on=match_cols,
    how="inner",
    validate="one_to_one")
dfp.sort_values(
...
conc_arr = pd.to_numeric(
    dfp["Total_Y_Concentration_cm3"],
    errors="coerce"
).to_numpy(dtype=float)
print("Matched CDP legs:", len(dfp))
print("Mass entries:", len(mass_arr))
print("Slope entries:", len(slope_arr))
print("Concentration entries:", len(conc_arr))
#%%
# CDP 3-panel seasonal trend
df_mass_slope = df_mass.copy()
df_conc = pd.DataFrame(total_concentration_cm3_mass100gone).copy()
...
dfp = dfp[
    dfp["Date"].str.startswith("2020-")
    & dfp["Month"].isin([2, 3, 8, 9])
].copy()
dfp.sort_values(
    ["Date_dt", "BCB_start"],
    kind="mergesort",
