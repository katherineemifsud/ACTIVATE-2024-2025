...
input_dir = '/home/disk/eos4/kathem24/activate/data/2020/FCDP'
FCDP = []
loaded_dates_FCDP = []
loaded_files_FCDP = []
...
for date in dates_legs:
    ...
    for file_path in fname_legs:
        ...
        for leg_num in [2, 3, 4, 6]:
            leg_key_name = f"LegIndex_{leg_num:02d}"

            selected_legs = df_legs[df_legs['LegIndex'] % 100 == leg_num]

            leg_dictionary[leg_key_name]['StartTimes'].extend(
                selected_legs['Time_Start'].tolist()
            )

            leg_dictionary[leg_key_name]['StopTimes'].extend(
                selected_legs['Time_Stop'].tolist()
            )
...
for i in range(len(dates_legs)):
    ...
    FCDP_bins = {f'FCDP_Bin{bin_label:02d}': np.array(FCDP_flight[col], dtype=float)
    for bin_label, col in enumerate(bin_name_FCDP)}
    ...
    for k in range(len(ACB_start)):
        ...
        FCDP_indices_in_range = np.where((FCDP_times >= start_time) & (FCDP_times <= end_time))[0]

        for FCDP_idx in zip(FCDP_indices_in_range):
            lwc_val = FCDP_lwc[FCDP_idx]

            if lwc_val >= 0.00001:
                calc_entry = {
                    'Date': date,
                    'Time': FCDP_times[FCDP_idx],
                ...
                for bin_label in range(0, len(bin_name_FCDP)):
                    bin_key = f'Bin{bin_label}_concentration'

                    calc_entry[bin_key] = (
                        FCDP_bins[f'FCDP_Bin{bin_label:02d}'][FCDP_idx]
                    )
...
for i in range(len(dates_legs)):
    ...
    for leg_start, leg_stop in combined_legs:
        for k in range(len(leg_start)):
            ...
            for FCDP_idx in FCDP_indices_in_range:
                lwc_val = FCDP_lwc[FCDP_idx]

                if lwc_val >= 0.00001:  # Adjust LWC threshold as needed
                    calc_entry = {
                        'Date': date,
                        'Time': FCDP_times[FCDP_idx],
...
for i in range(len(dates_legs)):
    ...
    bin_widths = [bin_log_FCDP[bin_label] for bin_label in range(0, len(bin_name_FCDP))]
        ...
        for FCDP_idx in FCDP_indices_in_range:
            ...
            if lwc_val >= 0.00001:
                total_concentration = sum(
                    np.nan_to_num(FCDP_bins[f'FCDP_Bin{bin_label:02d}'][FCDP_idx]) * bin_width
                    for bin_label, bin_width in zip(range(0, len(bin_name_FCDP)), bin_widths)
                )
                calc_entry = {
                    'Date': date,
                    'Time': FCDP_times[FCDP_idx],
                    ...
                    'Leg_stop': end_time,
                    'CWC': lwc_val,
                    'Total_Concentration': total_concentration / 1e6  # Units: cm³
                }
                in_cloud_concentrations.append(calc_entry)
print(f"Number of in-cloud entries: {len(in_cloud_concentrations)}")
...
plt.xlabel('Nr+Nc /cm³', fontsize=19, fontweight='bold')
plt.ylabel('LWC g/m³', fontsize=19, fontweight='bold')
plt.title('Mean LWC\nFMAS 2020 (FCDP in cloud)', fontsize=18, fontweight='bold')
plt.tight_layout()
plt.show()
#%%
#%%
...
for i in range(len(dates_legs)):
    ...
    FCDP_lwc = FCDP_flight['LWC_FCDP'].values
    FCDP_bins= {
    f'FCDP_Bin{bin_label:02d}': np.array(FCDP_flight[col], dtype=float)
    for bin_label, col in enumerate(bin_name_FCDP)
}
    ...
    for k in range(len(BCB_start)):
        ...
        if FCDP_indices_in_range.size > 0 and TwoDS_indices_in_range.size > 0 and rh_indices_in_range.size > 0:
            ...
            for FCDP_idx, TwoDS_idx, rh_idx in zip(FCDP_indices_in_range, TwoDS_indices_in_range, rh_indices_in_range):
                ...
                data_labels_FCDP.append(label)

                bin_values_FCDP = [FCDP_bins[f'FCDP_Bin{bin_label:02d}'][FCDP_idx]  for bin_label in range(len(bin_name_FCDP))]
                BCB_means_FCDP.append(bin_values_FCDP)

            if BCB_means_FCDP:
                total_BCB_means_FCDP.append(BCB_means_FCDP)

            leg_info_FCDP.append({
                'Date': date,
                'BCB_start': start20,
                'BCB_stop': end20,
                'Data_Labels': data_labels_FCDP,
            })

    master_FCDP_BCB.append(total_BCB_means_FCDP)
...
for i in range(len(dates_legs)):
    ...
    for k in range(len(BCB_start)):
        ...
        bin_means_FCDP = {f'Bin{bin_label:02d}_Y_mean': [] for bin_label in range(18)}
        bin_means_FCDP.update({f'Bin{bin_label:02d}_N_mean': [] for bin_label in range(18)})
        bin_means_FCDP.update({'Date': date, 'BCB_start': start20, 'BCB_stop': end20})
        ...
        if FCDP_indices_in_range.size > 0 and TwoDS_indices_in_range.size > 0 and rh_indices_in_range.size >0:
            for fcdp_idx, twods_idx, rh_idx in zip(FCDP_indices_in_range, TwoDS_indices_in_range, rh_indices_in_range):
                ...
                for bin_label in range(18):
                    ...
                    bin_means_FCDP[bin_key].append(bins[f'FCDP_Bin{bin_label:02d}'][fcdp_idx])

       
        for bin_label in range(18):
            for label in ['Y', 'N']:
                bin_key = f'Bin{bin_label:02d}_{label}_mean'
                if bin_means_FCDP[bin_key]:
                    bin_means_FCDP[bin_key] = np.nanmean(bin_means_FCDP[bin_key])

        total_BCB_means_FCDP.append(bin_means_FCDP)

    master_FCDP_BCB.append(total_BCB_means_FCDP)
for item in master_FCDP_BCB:
    for bin_means_FCDP in item:
        print(f"Date: {bin_means_FCDP['Date']}, Start: {bin_means_FCDP['BCB_start']}, Stop: {bin_means_FCDP['BCB_stop']}")
        for bin_label in range(18):
            for label in ['Y', 'N']:
                bin_key = f'Bin{bin_label:02d}_{label}_mean'
                print(f"   {bin_key}: {bin_means_FCDP[bin_key]}")
#%%
master_FCDP_BCB = []
leg_info_FCDP = []
...
for leg in leg_info_FCDP:
    ...
    for label, rh in zip(leg['Data_Labels'], rh_leg_vals):
        if label == 'Y':
            rh_Y_values_FCDP.append(rh)
plt.figure(figsize=(8,6))
plt.hist(rh_Y_values_FCDP, bins=30, color='teal', edgecolor='black', alpha=0.7)
plt.axvline(95, color='red', linestyle='--', label='RH = 95% threshold')

plt.xlabel("Relative Humidity (%)")
plt.ylabel("Count")
plt.title("FCDP: RH Values for Seconds Labeled 'Y'")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
for flight_data in master_FCDP_BCB:
    for bin_means_FCDP in flight_data:
        ...
        for bin_label in range(18):
            ...
            # FCDP original: dN/dlog10D in #/m3
            # Step 1: multiply by dlogD -> #/m3 per bin
            # Step 2: divide by dD -> #/m3/um
            # Step 3: divide by 1e6 -> #/cm3/um

            Y_calc[bin_key_Y] = np.nanmean(bin_means_FCDP[bin_key_Y]) * Logg_FCDP[bin_label]/1e6

            N_calc[bin_key_N] = (np.nanmean(bin_means_FCDP[bin_key_N])* Logg_FCDP[bin_label]/1e6
            )
...
if 'Y_BCB_calc_FCDP' in globals():
    BCB_leg_keys = {(entry['Date'], entry['BCB_start'], entry['BCB_stop']) for entry in Y_BCB_calc_FCDP}

    filtered_Y_BCB_calc_FCDP = [entry for entry in Y_BCB_calc_FCDP if (entry['Date'], entry['BCB_start'], entry['BCB_stop']) in BCB_leg_keys]
    filtered_N_BCB_calc_FCDP = [entry for entry in N_BCB_calc_FCDP if (entry['Date'], entry['BCB_start'], entry['BCB_stop']) in BCB_leg_keys]

    print(f"Original Y_BCB_calc_FCDP legs: {len(Y_BCB_calc_FCDP)}")
    print(f"Filtered Y_BCB_calc_FCDP legs: {len(filtered_Y_BCB_calc_FCDP)}")
    print(f"Original N_BCB_calc_FCDP legs: {len(N_BCB_calc_FCDP)}")
    print(f"Filtered N_BCB_calc_FCDP legs: {len(filtered_N_BCB_calc_FCDP)}")

    Y_BCB_calc_FCDP = filtered_Y_BCB_calc_FCDP
    N_BCB_calc_FCDP = filtered_N_BCB_calc_FCDP
else:
...
for entry in Y_BCB_calc_FCDP:
    bin_keys = [f'Bin{bin_label}_Y_mean' for bin_label in range(0, 18)]
