...
#%%
#Import the instrument data for the CDP
...
#continue using the CDP for lwc and pull CAS concentrations but filter 2DS N also to get drizzle drops out 
        ...
        for k in range(len(leg_start)):
            ...
            for CDP_idx in CDP_indices_in_range:
                ...
                if lwc_val >= 0.01:  # Adjust LWC threshold as needed
                    ...
                    # Include droplet concentrations from CDP bins
                    for bin_label in range(30):
                        bin_key = f'Bin{bin_label}_concentration'
                        calc_entry[bin_key] = CDP_bins[f'CDP_Bin{bin_label:02d}'][CDP_idx]
...
for i in range(len(dates_legs)):
    ...
    bin_widths = [bin_log_CDP[bin_label] for bin_label in range(30)]
        ...
        for CDP_idx in CDP_indices_in_range:
            ...
            if lwc_val >= 0.01:
                ...
                total_concentration = sum(
                    ...
                    for bin_label, bin_width in zip(range(30), bin_widths)
                )
...
plt.ylim(10**-2, 10**-0.5)  # Adjust if needed
...
plt.scatter(concentration, total_liquid_water_values, c=rwc_percentage, 
            cmap='RdBu_r', alpha=0.8, edgecolors='k', linewidths=0.3, s=15)  # Darker scatter points
...
for idx, entry in enumerate(total_combined_concentration):  
    ...
    for i in range(len(dates_legs)):
        if dates_legs[i] == matching_date:
            cdp_flight = CDP_1Hz[i]  # Get the correct CDP flight dataset
            break
    
    if cdp_flight is not None:
        # Convert CDP times to numeric if not already
        cdp_flight['Time_Start'] = pd.to_numeric(cdp_flight['Time_Start'], errors='coerce')
        
        # Find the matching row in CAS_flight
        matching_cdp_row = cdp_flight.loc[cdp_flight['Time_Start'] == matching_time]
        
        if not matching_cdp_row.empty:
            reff_value = matching_cdp_row['Reff_CDP'].values[0]  # Extract Reff_CDP
        else:
    ...
    entry['Reff_CDP'] = reff_value  

# Debugging: Check if values are stored
print(f"Extracted {len(total_combined_concentration)} effective radii.")
print(f"First 5 entries with radii: {[entry['Reff_CDP'] for entry in total_combined_concentration[:5]]}")


# %%
# Compute Deff_CDP from Reff_CDP
for entry in total_combined_concentration:
    if 'Reff_CDP' in entry:  # Ensure the radius exists before multiplying
        entry['Deff_CDP'] = entry['Reff_CDP'] * 2  # Convert radius to diameter

#%%
# Extract Deff_CAS from total_combined_concentration
deff_values = [entry['Deff_CDP'] for entry in total_combined_concentration if 'Deff_CDP' in entry]

# Debugging: Check extracted Deff_CDP values
...
# Scatterplot with refined diameter contours
fig, ax = plt.subplots(figsize=(8, 6))

# **1️⃣ Contour lines for Effective Diameter (plotted first, so scatter is on top)**
contour_levels = np.linspace(np.nanmin(filtered_deff_values), np.nanmax(filtered_deff_values), 7)  # Fewer levels

contour = ax.tricontour(filtered_concentration, filtered_liquid_water_values, 
                        filtered_deff_values, levels=contour_levels, 
                        colors='gray', linewidths=0.8, alpha=0.3)  # **Thinner, more transparent**

# **2️⃣ Scatterplot of RWC% (Plotted on top)**
sc = ax.scatter(filtered_concentration, filtered_liquid_water_values, 
                c=filtered_rwc_percentage, cmap='RdBu_r', alpha=0.9, 
                edgecolors='k', linewidths=0.5, s=15)  # **Darker points with black edges**

# **3️⃣ Colorbar for RWC%**
cbar_rwc = plt.colorbar(sc, ax=ax, pad=0.08)  # **More spacing**
cbar_rwc.set_label("RWC/LWC (%)", fontsize=12, labelpad=15)

# **4️⃣ Properly Shaded Colorbar for Effective Diameter**
sm = plt.cm.ScalarMappable(cmap="Greys", norm=mcolors.Normalize(vmin=np.nanmin(filtered_deff_values), vmax=np.nanmax(filtered_deff_values)))
sm.set_array([])  # Dummy data for the colorbar

cbar_deff = plt.colorbar(sm, ax=ax, pad=0.12)  # **More spacing between the two bars**
cbar_deff.set_label("Effective Diameter (μm)", fontsize=12, labelpad=15)

# **5️⃣ Log Scale & Labels**
ax.set_xscale('log')
ax.set_yscale('log')
ax.set_xlabel('Nr+Nc /cm³', fontsize=16, fontweight='bold')
ax.set_ylabel('LWC g/m³', fontsize=16, fontweight='bold')
...
# Create DataFrame
df_CDP_rainwater = pd.DataFrame({
    'RWC': [entry['RWC'] for entry in total_liquid_water],
    'Deff_CDP': [entry['Deff_CDP'] for entry in total_combined_concentration]  # Make sure 'Deff_CDP' exists
})

# Ensure no NaN values before binning
df_CDP_rainwater = df_CDP_rainwater.dropna(subset=['RWC', 'Deff_CDP'])
...
# Assign bin labels
df_CDP_rainwater['RWC_Category'] = pd.cut(df_CDP_rainwater['RWC'], bins=bins, labels=labels, include_lowest=True)

# Verify if RWC_Category was successfully created
print(df_CDP_rainwater[['RWC', 'RWC_Category']].head())

# Remove NaN values again to ensure no issues
df_CDP_rainwater = df_CDP_rainwater.dropna()

# Check if there are any valid rows left after filtering
if df_CDP_rainwater.empty:
    print("⚠️ No valid data after filtering. Check RWC values!")
else:
    # Create the Boxplot
    plt.figure(figsize=(8, 6))
    sns.boxplot(x='RWC_Category', y='Deff_CDP', data=df_CDP_rainwater, palette='Blues')

    # Customize plot
    plt.xlabel('(RWC) g/m^3', fontsize=14, fontweight='bold')
    plt.ylabel('Effective Diameter (μm)', fontsize=14, fontweight='bold')
    plt.title('CDP in-cloud January-June 2022', fontsize=16, fontweight='bold')
...
sns.boxplot(x='RWC_Category', y='Deff_CDP', data=df_CDP_rainwater, palette='Blues', width=0.6, fliersize=0)

# Swarmplot (individual data points)
sns.swarmplot(x='RWC_Category', y='Deff_CDP', data=df_CDP_rainwater, color='black', alpha=0.5, size=3)
...
sns.violinplot(x='RWC_Category', y='Deff_CDP', data=df_CDP_rainwater, palette='Blues', inner="quartile", cut=0)
...
df = pd.DataFrame({
    'Deff_CDP': [entry['Deff_CDP'] for entry in total_combined_concentration],
...
X = df[['Deff_CDP', 'Total_Concentration', 'CWC']]
y = df['RWC']
