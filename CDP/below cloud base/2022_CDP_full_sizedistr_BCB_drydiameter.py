...
#     times = CDP_flight.Time_Start.values
...
#     CDP_indices = []
#     twods_indices = []
#     print(f"Processing Date: {flight_date}, BCB Start: {BCB_start}, BCB Stop: {BCB_stop}")

#     for j, time in enumerate(times):
#         if time in times1:
#             CDP_indices.append(j)
...
#                 bin_values = [
#                     CDP_flight.CDP_Bin00.values[CDP_indices[j]], CDP_flight.CDP_Bin01.values[CDP_indices[j]],
#                     CDP_flight.CDP_Bin02.values[CDP_indices[j]], CDP_flight.CDP_Bin03.values[CDP_indices[j]],
#                     CDP_flight.CDP_Bin04.values[CDP_indices[j]], CDP_flight.CDP_Bin05.values[CDP_indices[j]],
#                     CDP_flight.CDP_Bin06.values[CDP_indices[j]], CDP_flight.CDP_Bin07.values[CDP_indices[j]],
#                     CDP_flight.CDP_Bin08.values[CDP_indices[j]], CDP_flight.CDP_Bin09.values[CDP_indices[j]],
#                     CDP_flight.CDP_Bin10.values[CDP_indices[j]], CDP_flight.CDP_Bin11.values[CDP_indices[j]],
#                     CDP_flight.CDP_Bin12.values[CDP_indices[j]], CDP_flight.CDP_Bin13.values[CDP_indices[j]],
#                     CDP_flight.CDP_Bin14.values[CDP_indices[j]], CDP_flight.CDP_Bin15.values[CDP_indices[j]],
#                     CDP_flight.CDP_Bin16.values[CDP_indices[j]], CDP_flight.CDP_Bin17.values[CDP_indices[j]],
#                     CDP_flight.CDP_Bin18.values[CDP_indices[j]], CDP_flight.CDP_Bin19.values[CDP_indices[j]],
#                     CDP_flight.CDP_Bin20.values[CDP_indices[j]], CDP_flight.CDP_Bin21.values[CDP_indices[j]],
#                     CDP_flight.CDP_Bin22.values[CDP_indices[j]], CDP_flight.CDP_Bin23.values[CDP_indices[j]],
#                     CDP_flight.CDP_Bin24.values[CDP_indices[j]], CDP_flight.CDP_Bin25.values[CDP_indices[j]],
#                     CDP_flight.CDP_Bin26.values[CDP_indices[j]], CDP_flight.CDP_Bin27.values[CDP_indices[j]],
#                     CDP_flight.CDP_Bin28.values[CDP_indices[j]], CDP_flight.CDP_Bin29.values[CDP_indices[j]]
#                 ]
#                 BCB_means.append(bin_values)

#             if BCB_means:
#                 BCB_means = np.mean(BCB_means, axis=0) 
...
#     Bin00 = CDP_flight.CDP_Bin00.values[CDP_indices]
#     Bin01 = CDP_flight.CDP_Bin01.values[CDP_indices]
#     Bin02 = CDP_flight.CDP_Bin02.values[CDP_indices]
#     Bin03 = CDP_flight.CDP_Bin03.values[CDP_indices]
#     Bin04 = CDP_flight.CDP_Bin04.values[CDP_indices]
#     Bin05 = CDP_flight.CDP_Bin05.values[CDP_indices]
#     Bin06 = CDP_flight.CDP_Bin06.values[CDP_indices]
#     Bin07 = CDP_flight.CDP_Bin07.values[CDP_indices]
#     Bin08 = CDP_flight.CDP_Bin08.values[CDP_indices]
#     Bin09 = CDP_flight.CDP_Bin09.values[CDP_indices]
#     Bin10 = CDP_flight.CDP_Bin10.values[CDP_indices]
#     Bin11 = CDP_flight.CDP_Bin11.values[CDP_indices]
#     Bin12 = CDP_flight.CDP_Bin12.values[CDP_indices]
#     Bin13 = CDP_flight.CDP_Bin13.values[CDP_indices]
#     Bin14 = CDP_flight.CDP_Bin14.values[CDP_indices]
#     Bin15 = CDP_flight.CDP_Bin15.values[CDP_indices]
#     Bin16 = CDP_flight.CDP_Bin16.values[CDP_indices]
#     Bin17 = CDP_flight.CDP_Bin17.values[CDP_indices]
#     Bin18 = CDP_flight.CDP_Bin18.values[CDP_indices]
#     Bin19 = CDP_flight.CDP_Bin19.values[CDP_indices]
#     Bin20 = CDP_flight.CDP_Bin20.values[CDP_indices]
#     Bin21 = CDP_flight.CDP_Bin21.values[CDP_indices]
#     Bin22 = CDP_flight.CDP_Bin22.values[CDP_indices]
#     Bin23 = CDP_flight.CDP_Bin23.values[CDP_indices]
#     Bin24 = CDP_flight.CDP_Bin24.values[CDP_indices]
#     Bin25 = CDP_flight.CDP_Bin25.values[CDP_indices]
#     Bin26 = CDP_flight.CDP_Bin26.values[CDP_indices]
#     Bin27 = CDP_flight.CDP_Bin27.values[CDP_indices]
#     Bin28 = CDP_flight.CDP_Bin28.values[CDP_indices]
#     Bin29 = CDP_flight.CDP_Bin29.values[CDP_indices]
...
#                     bin_value = CDP_flight[f'CDP_Bin{bin_label:02d}'].values[CDP_indices[j]]
...
#                     print(f"   Bin{bin_label:02d}_{label} Mean: {mean_value}")
#                 else:
#                     print(f"   Bin{bin_label:02d}_{label} Mean: No valid data")
#%%
master_CDP_BCB = []
leg_info = []
...
for i in range(len(dates_legs)):
    ...
    # Align timestamps more efficiently
    times_dict = {time: idx for idx, time in enumerate(times1)}
    aligned_indices = [(idx, times_dict[time]) for idx, time in enumerate(times) if time in times_dict]

    CDP_indices, twods_indices = zip(*aligned_indices) if aligned_indices else ([], [])
    ...
    bins = {key: values[CDP_indices] for key, values in bins.items()}
    ...
    for k in range(len(BCB_start)):
        ...
        # Efficiently find start and end indices in aligned_times
...
for i in range(len(dates_legs)):
    ...
    # Convert time and values to numpy arrays
    times = np.array(CDP_flight.Time_Start.values, dtype=float)
    times1 = np.array(twoDS_flight.Time_Start.values, dtype=float)
    lwc = np.array(CDP_flight.LWC_CDP.values, dtype=float)
    N_total = np.array(twoDS_flight['N-total_2DS'].values, dtype=float)

    # Debugging: Print the range of times for each dataset
    print(f"Processing {flight_date}:")
    print(f"  TwoDS Times Range: {times1.min()} to {times1.max()}")
    print(f"  CDP Times Range: {times.min()} to {times.max()}")
    ...
    if not aligned_indices:
        print(f"  Warning: No aligned indices found for {flight_date}")
        continue
    ...
    # Debugging: Print alignment results
    print(f"  Aligned CDP Indices: {CDP_indices[:10]}... (total {len(CDP_indices)})")
    print(f"  Aligned TwoDS Indices: {twods_indices[:10]}... (total {len(twods_indices)})")

    # Pre-fetch all bin values
    bins = {
        f'CDP_Bin{bin_label:02d}': np.array(CDP_flight[f'CDP_Bin{bin_label:02d}'].values, dtype=float)[CDP_indices]
        for bin_label in range(30)
    }
    ...
    for k in range(len(BCB_start)):
        ...
        if len(indices_in_range) == 0:
            print(f"  No indices found in range {start20} to {end20} for {flight_date}")
            continue
...
print(f"\nProcessed all dates. Total flights: {len(dates_legs)}")
for item in master_CDP_BCB:
    for bin_means in item:
        print(f"Date: {bin_means['Date']}, Start: {bin_means['BCB_start']}, Stop: {bin_means['BCB_stop']}")
        for bin_label in range(30):
...
#%%
# Constants
MATCH_TOLERANCE = 0.1  # Adjust tolerance if necessary
...
for i in range(len(dates_legs)):
    ...
    # Align times with tolerance
    aligned_indices = []
    for start, stop in zip(BCB_start, BCB_stop):
        indices = np.where((times >= start - MATCH_TOLERANCE) & (times <= stop + MATCH_TOLERANCE))[0]
        if len(indices) > 0:
            aligned_indices.append(indices)

    # Collect matched times
    matched_CDP_times = [times[indices] for indices in aligned_indices if len(indices) > 0]
    matched_TwoDS_times = [
        times1[np.where((times1 >= start - MATCH_TOLERANCE) & (times1 <= stop + MATCH_TOLERANCE))[0]]
        for start, stop in zip(BCB_start, BCB_stop)
    ]

    # Debugging Output
    print(f"Processing Date: {flight_date}")
    print(f"BCB Start/Stop Times: {BCB_start} to {BCB_stop}")
    print(f"Matched CDP Times (First 5): {[t[:5] for t in matched_CDP_times if len(t) > 0]}")
    print(f"Matched TwoDS Times (First 5): {[t[:5] for t in matched_TwoDS_times if len(t) > 0]}")

    # If aligned times are empty, log a warning
    if not matched_CDP_times or not matched_TwoDS_times:
        print(f"WARNING: No matching data for BCB intervals on {flight_date}")
#%%
# Recreating the old code with the fixed alignment logic
master_CDP_BCB = []
leg_info = []
...
for i in range(len(dates_legs)):
    ...
    # Efficiently align timestamps using the corrected logic
    times1_mapping = {time: idx for idx, time in enumerate(times1)}  # Dictionary for fast lookup
    CDP_indices = [idx for idx, time in enumerate(times) if time in times1_mapping]
    twods_indices = [times1_mapping[time] for time in times if time in times1_mapping]
    ...
    for k in range(len(BCB_start)):
        ...
        # Find start and end indices in aligned times
        interval_indices = np.where((aligned_times >= start20) & (aligned_times <= end20))[0]
...
for i in range(len(dates_legs)):
    ...
    # Align timestamps more efficiently using rounding for consistency
    times_rounded = np.round(times, decimals=3)
    times1_rounded = np.round(times1, decimals=3)

    times1_mapping = {time: idx for idx, time in enumerate(times1_rounded)}
    aligned_indices = [
        (idx, times1_mapping[time])
        for idx, time in enumerate(times_rounded)
        if time in times1_mapping
    ]
    ...
    # Pre-fetch all bin values
    bins = {
        f'CDP_Bin{bin_label:02d}': np.array(CDP_flight[f'CDP_Bin{bin_label:02d}'].values, dtype=float)
        for bin_label in range(30)
    }
    bins = {key: values[CDP_indices] for key, values in bins.items()}
    ...
    for k in range(len(BCB_start)):
        ...
        # Efficiently find indices within the BCB interval
        indices_in_range = np.where((aligned_times >= start20) & (aligned_times <= end20))[0]
...
for i in range(len(dates_legs)):
    ...
    times = np.round(CDP_flight.Time_Start.values, 3)
    times1 = np.round(twoDS_flight.Time_Start.values, 3)
    ...
    for k in range(len(BCB_start)):
        ...
        if indices_in_range.size > 0:
            ...
            print(f"WARNING: No data found for interval {start20} to {end20}.")
...
#%%
# Use np.round to handle fractional precision
cdp_rounded_times = np.round(times, 3)  # Keep precision to three decimal places
twods_rounded_times = np.round(times1, 3)  # Do the same for TwoDS times

print("Rounded CDP Times (First 10):", cdp_rounded_times[:10])
print("Rounded TwoDS Times (First 10):", twods_rounded_times[:10])

# Create a mapping for TwoDS times with indices for fast lookup
times_dict = {time: idx for idx, time in enumerate(twods_rounded_times)}

# Align CDP and TwoDS times
aligned_indices = [
    (idx, times_dict[time]) for idx, time in enumerate(cdp_rounded_times) if time in times_dict
]
...
if aligned_indices:
    ...
    CDP_indices, twods_indices = [], []

print("Aligned CDP Indices:", CDP_indices[:10])
print("Aligned TwoDS Indices:", twods_indices[:10])
#%%
# Debugging the first leg of 2022-01-11
date_index = dates_legs.index("2022-01-11")
leg_dict = leg_data[date_index]

# Get the first BCB_start and BCB_stop
BCB_start = float(leg_dict['LegIndex_02']['StartTimes'][0])  # First start time
BCB_stop = float(leg_dict['LegIndex_02']['StopTimes'][0])  # First stop time

# Extract CDP and TwoDS times
CDP_flight = CDP[date_index]
twoDS_flight = twoDS[date_index]

# Convert times
CDP_times = np.array(CDP_flight.Time_Start.values, dtype=float)
TwoDS_times = np.array(twoDS_flight.Time_Start.values, dtype=float)

# Round times
rounded_CDP_times = np.round(CDP_times, 3)
rounded_TwoDS_times = np.round(TwoDS_times, 3)

# Check matching in CDP
CDP_indices_in_range = np.where((rounded_CDP_times >= BCB_start) & (rounded_CDP_times <= BCB_stop))[0]
matching_CDP_times = rounded_CDP_times[CDP_indices_in_range]

# Check matching in TwoDS
TwoDS_indices_in_range = np.where((rounded_TwoDS_times >= BCB_start) & (rounded_TwoDS_times <= BCB_stop))[0]
matching_TwoDS_times = rounded_TwoDS_times[TwoDS_indices_in_range]

# Print results
print(f"Inspecting Date: 2022-01-11, First Leg")
print(f"BCB Start: {BCB_start}, BCB Stop: {BCB_stop}")
print(f"Matching CDP Times: {matching_CDP_times}")
print(f"Matching TwoDS Times: {matching_TwoDS_times}")
print(f"Number of Matching CDP Times: {len(matching_CDP_times)}")
print(f"Number of Matching TwoDS Times: {len(matching_TwoDS_times)}")

#%%
# Match BCB_start and BCB_stop to CDP and TwoDS Times
BCB_start = 49898.0  # Example Start
BCB_stop = 50084.0   # Example Stop

# Rounded/Floored times for alignment
rounded_CDP_times = np.floor(np.array(CDP_flight.Time_Start.values, dtype=float))
rounded_TwoDS_times = np.floor(np.array(twoDS_flight.Time_Start.values, dtype=float))

# Find indices within the BCB range
CDP_indices_in_range = np.where((rounded_CDP_times >= BCB_start) & (rounded_CDP_times <= BCB_stop))[0]
TwoDS_indices_in_range = np.where((rounded_TwoDS_times >= BCB_start) & (rounded_TwoDS_times <= BCB_stop))[0]

# Extract matching times
matching_CDP_times = rounded_CDP_times[CDP_indices_in_range]
matching_TwoDS_times = rounded_TwoDS_times[TwoDS_indices_in_range]

# Print results
print(f"BCB Start: {BCB_start}, BCB Stop: {BCB_stop}")
print(f"Matching CDP Times: {matching_CDP_times[:10]} ... Total: {len(matching_CDP_times)}")
print(f"Matching TwoDS Times: {matching_TwoDS_times[:10]} ... Total: {len(matching_TwoDS_times)}")
...
for entry in Y_BCB_calc:
    bin_means = []
    for i in range(0, 30):  # Bins from 0 to 29
        key = f'Bin{i}_Y_mean'
...
for entry in Y_BCB_calc:
    bin_means = []
    for i in range(0, 30):  # Bins from 12 to 29
        key = f'Bin{i}_Y_mean'
        bin_means.append(entry.get(key, np.nan))
    all_bin_means.append({
        'Date': entry['Date'],
...
# Define the size distribution functionCORRECT
def size_distribution(ddry, N0, D):
    return N0 * np.exp(-ddry / D)
