...
#%%
#as a heatmap
# === CDP Heatmap Plot Using Real Y_CDP_calc ===
from matplotlib.colors import LinearSegmentedColormap, LogNorm
import numpy.ma as ma
from scipy.interpolate import interp1d

# Create faded viridis colormap
...
# Compute and print median error per windspeed bin
...
#%%
#counting error 
# CDP Median Counting Errors (use your previously computed values)
median_relative_errors_CDP = [0.0240, 0.0317, 0.0230, 0.0219, 0.0193, 0.0162]

# Prepare counting error bars for CDP
counting_errors_CDP = np.array(median_relative_errors_CDP) * total_concentrations_CDP
...
#%%
#counting error
# === CDP Median Relative Counting Errors (same bins as before) ===
median_relative_errors_mass_CDP = [0.0240, 0.0317, 0.0230, 0.0219, 0.0193, 0.0162]
counting_errors_mass_CDP = np.array(median_relative_errors_mass_CDP) * total_mass_values_CDP
...
plt.errorbar(windspeed_values_mass_CDP, total_mass_values_CDP,
             ...
             label="Standard Error", zorder=3)
plt.errorbar(windspeed_values_mass_CDP, total_mass_values_CDP,
             yerr=counting_errors_mass_CDP, fmt='none',
             ecolor='#8c510a', elinewidth=3, capsize=5, capthick=2,
             label="CDP Counting Error", zorder=2)
x_fit_mass_CDP = np.linspace(min(windspeed_values_mass_CDP), max(windspeed_values_mass_CDP), 100)
y_fit_mass_CDP = linear_model(x_fit_mass_CDP, *popt_mass_CDP)
plt.plot(x_fit_mass_CDP, y_fit_mass_CDP, '-', color='blue', linewidth=2.5,
         label=f'Fit: y = ({m_fit_mass_CDP:.3f}±{m_err_mass_CDP:.3f})x + {b_fit_mass_CDP:.3f}\nR² = {r_squared_mass_CDP:.2f}, R = {r_value_mass_CDP:.2f}')
plt.xlabel("Wind Speed (m s$^{-1}$)", fontsize=20, fontweight='bold')
