...
import pickle
#%%
with open("FCDP_slope_massLE1002022.pkl", "rb") as f:
...
df_fcdp = df_fcdp[df_fcdp["Date"].astype(str).str.startswith("2022-")].copy()
df_fcdp["Month"] = df_fcdp["Date"].astype(str).str[5:7].astype(int)
df_fcdp = df_fcdp[df_fcdp["Month"].isin([1, 2, 3, 4, 5, 6])].copy()
...
plt.title("Dry Mass Timeline (January-June 2022)\nLegs ordered by Date then BCB_start",
...
df_fcdp = pd.DataFrame(filtered_slope_mass100gone).copy()
df_fcdp = df_fcdp[
    df_fcdp["Date"].astype(str).str.startswith("2022-")
].copy()
df_fcdp["Month"] = (
    df_fcdp["Date"]
    .astype(str)
...
df_fcdp = df_fcdp[
    df_fcdp["Month"].isin([1, 2, 3, 4, 5, 6])
].copy()
bad_mass_mask = (
    pd.to_numeric(
...
plt.title("FCDP BCB Mass January-June 2021", fontsize=18, fontweight="bold")
...
plt.ylabel("GCCN Mass (µg/m³)", fontsize=16, fontweight="bold")
plt.xlabel("Leg index", fontsize=16, fontweight="bold")
plt.title("FCDP BCB Mass January-June 2022\nMonthly Means", fontsize=18, fontweight="bold")
plt.legend(ncol=2, fontsize=10)
plt.yticks(fontsize=14, fontweight="bold")
plt.xticks(fontsize=14, fontweight="bold")
plt.tight_layout()
...
ax.set_title("FCDP BCB Mass January-June 2021\nMonthly Means",
...
plt.title("FCDP BCB Slope January-June 2022",
          fontsize=18, fontweight="bold")
plt.tight_layout()
plt.show()
# %%
...
plt.ylabel("Slope (D)", fontsize=16, fontweight="bold")
plt.xlabel("Leg index", fontsize=16, fontweight="bold")
plt.title("FCDP BCB Slope (D) January-June 2022",
          fontsize=18, fontweight="bold")
plt.legend(ncol=2, fontsize=10)
plt.yticks(fontsize=14, fontweight="bold")
plt.xticks(fontsize=14, fontweight="bold")
...
plt.xlabel("Leg index", fontsize=16, fontweight="bold")
plt.title("FCDP BCB Dry Slope (D) January-June 2022\nMonthly Means", fontsize=18, fontweight="bold")
plt.legend(ncol=2, fontsize=10)
plt.yticks(fontsize=14, fontweight="bold")
plt.xticks(fontsize=14, fontweight="bold")
plt.tight_layout()
...
ax.set_title("FCDP BCB Slope (D) January-June 2022\nMonthly Trend", fontsize=18, fontweight="bold")
ax.legend(ncol=2, fontsize=10, loc="upper right")
ax.set_xticks(tick_pos)
...
ax.set_xlabel("Flight Date", fontsize=20, fontweight="bold")
ax.set_title("FCDP BCB Slope January-June 2021\nMonthly Trend",
             fontsize=20, fontweight="bold")
ax.set_xticks(tick_pos)
ax.set_xticklabels(tick_lab, rotation=60, ha="right", fontsize=7, fontweight="bold")
labels = ax.get_xticklabels()
...
#%%
#concentration 
with open("FCDP_concentration_massLE1002022.pkl", "rb") as f:
...
dfp_fcdp = dfp_fcdp[dfp_fcdp["Date"].astype(str).str.startswith("2022-")].copy()
dfp_fcdp["Month"] = dfp_fcdp["Date"].astype(str).str[5:7].astype(int)
dfp_fcdp = dfp_fcdp[dfp_fcdp["Month"].isin([1, 2, 3, 4, 5, 6])].copy()
dfp_fcdp["Date_dt"] = pd.to_datetime(dfp_fcdp["Date"])
sort_cols = ["Date_dt"]
if "BCB_start" in dfp_fcdp.columns:
    sort_cols.append("BCB_start")
...
ax.set_xlabel("Flight Date", fontsize=20, fontweight="bold")
ax.set_title("FCDP Concentration January-June 2021",
             fontsize=20, fontweight="bold")
ax.set_xticks(tick_pos)
ax.set_xticklabels(tick_lab, rotation=75, ha="right", fontsize=8, fontweight="bold")
labels = ax.get_xticklabels()
...
with open("FCDP_3panel_seasonaltrend_FMAS2022.pkl", "wb") as f:
    pickle.dump(fcdp_plot_data, f)
print("Saved FCDP 3-panel plot data.")
# %%
#CAS, CDP, and FCDP 3-panel plot comparison
with open("CAS_3panel_seasonaltrend_2021.pkl", "rb") as f:
    cas = pickle.load(f)
with open("CDP_3panel_seasonaltrend_2021.pkl", "rb") as f:
    cdp = pickle.load(f)
with open("FCDP_3panel_seasonaltrend_FMAS2022.pkl", "rb") as f:
    fcdp = pickle.load(f)
month_name = {
    1: "January",
    2: "February",
...
titles = [
    "2021 CAS, CDP, and FCDP GCCN mass seasonal trend over the Western Atlantic",
    "2021 CAS, CDP, and FCDP GCCN slope parameter seasonal trend over the Western Atlantic",
    "2021 CAS, CDP, and FCDP total GCCN concentration seasonal trend over the Western Atlantic"
]
...
with open("CAS_CDP_FCDP_3panel_seasonaltrend_2021.pkl", "wb") as f:
