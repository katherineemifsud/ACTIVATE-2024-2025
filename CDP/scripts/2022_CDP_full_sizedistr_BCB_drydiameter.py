    ...
    for file_path in fname_twoDS:
        ...
        for bin_ in bin_name:
            if bin_ in df_2DS.columns:
                ...
                df_2DS.replace(-9999, 0, inplace=True)
...
for entry in Y_BCB_calc:
    # Initialize a list to hold bin means
    bin_means = []
    for i in range(0, 30):  # Iterate over bin indices
        key = f'Bin{i:02d}_Y_mean'  # Match the exact key format in Y_BCB_calc
        if key in entry:  # Ensure the key exists
            bin_means.append(entry[key])  # Add the bin mean value
        else:
            bin_means.append(np.nan)  # Add NaN if the key is missing
...
for entry in all_bin_means:
    ...
    valid_indices = ~np.isnan(bin_means)  # Filter valid data
    date = entry['Date']
...
# Initialize the color map for plotting
unique_dates = sorted(set(entry['Date'] for entry in all_bin_means))
colors = cm.viridis(np.linspace(0, 1, len(unique_dates)))
date_color_map = {date: color for date, color in zip(unique_dates, colors)}
...
for entry in all_bin_means:
    ...
    try:
        ...
        # Store the results in master_BCB_exponential
...
plt.title('Below Cloud Base clear sky distribution \n January-June 2022', fontsize=12, fontweight='bold')
plt.xticks(np.arange(0, 50, 5))
plt.legend(title='Date', ncol=7, loc='upper center', bbox_to_anchor=(0.5, -0.15), fancybox=True, shadow=True)
plt.tight_layout()
plt.show()
...
for i in range(len(dates_legs)):
    ...
    master_BCB_CDP.append(all_BCB_means) #List that contains all Min flights  
#%%
Z0 = 0.02  # meters 
...
for i, flight in enumerate(master_BCB_CDP):
    for j, wind_alt in enumerate(flight):
        try:
            ...
            rh_mean = master_BCB_RH_CDP[i][j]['Rh_mean'][0]
...
for i, flight in enumerate(master_BCB_CDP):
    for j, wind_alt in enumerate(flight):
        try:
            ...
            if date in master_BCB_exponential:
                exp_params_list = master_BCB_exponential[date]
                gRh_mean = master_BCB_gRH_CDP[i][j]['gRh_mean'][0]  # Assume matching index
...
# Iterate over each date in master_BCB_exponential and corresponding legs in master_BCB_gRH_CDP
for i, (date, legs_exponential) in enumerate(master_BCB_exponential.items()):
    # Ensure that we have a corresponding entry in master_BCB_gRH_CDP
    if i >= len(master_BCB_gRH_CDP):
        print(f"No corresponding gRh data for date {date}, skipping...")
        continue
...
# Iterate over each date in master_min_exponential and corresponding legs in master_BCB_gRH_CDP
for i, (date, legs_exponential) in enumerate(master_BCB_exponential.items()):
    # Ensure that we have a corresponding entry in master_BCB_gRH_CDP
    if i >= len(master_BCB_gRH_CDP):
        print(f"No corresponding gRh data for date {date}, skipping...")
    ...
    for j, (leg_exponential, leg_grh) in enumerate(zip(legs_exponential, legs_grh)):
        ...
        # Store the results in master_BCB_ddry_CDP
        master_BCB_ddry_CDP.append({
            'Date': date,
            'Leg_index': j,
            'ddry': ddry_values,
