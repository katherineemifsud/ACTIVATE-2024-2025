...
bin_radius=[1.875, 2.625, 3.5, 4.5, 5.5, 6.5, 7.5, 8.5, 9.75, 11.25, 12.75, 
             14.25, 15.75, 17.25, 18.75, 20.25, 22, 24 ]
# %%
D12 = (4.5-3)
D13 = (6-4.5)
D14 = (8-6)
...
for date in dates_FCDP:
    ...
    for file_path in fname_FCDP:
        ...
        df_FCDP['ED_FCDP']=pd.to_numeric(df_FCDP['ED_FCDP'], errors='coerce')
...
for i in range(len(dates_legs)):
    ...
    eff_diam = FCDP_flight.ED_FCDP.values
    ...
    for i in range(len(BCB_start)):
        ...
        if index7_start is not None and index7_end is not None:
            for i in range(index7_start, index7_end + 1):
                lwc_val = lwc[i]
                effec_val = eff_diam[i]
                lwc_label = 'Y' if 0 <= lwc_val <= 0.0000025 else 'N'
                effec_label = 'Y' if 0 <= effec_val <= 0.00002 else 'N'
                if lwc_label == 'Y' and effec_label == 'Y':
                    label = 'Y'
                else:
                    label = 'N'
    ...
    if BCB_start_list and BCB_stop_list:
        ...
        for idx, t in enumerate(times):
            if BCB_start <= int(t) <= BCB_stop:
                lwc_val = lwc[idx]
                effec_val = eff_diam[idx]
                print(f"LWC value at index {idx}: {lwc_val}")
                print(f"Effective diameter at index {idx}: {effec_val}")
                lwc_category = 'Y' if 0 <= lwc_val <= 0.0000025 else 'N'
                category_effec = 'Y' if 0 <= effec_val <= 0.00002 else 'N'
                if lwc_category == 'N' or category_effec == 'N':
                    found_N_values = True

                category = 'Y' if lwc_category == 'Y' and category_effec == 'Y' else 'N'
...
plt.ylabel('Droplet concentration (dN/dlogd) /m^3',  fontsize=16, fontweight='bold')
...
# Convert Y_values and N_values from /m^3 to /cm^3
Y_values_cm3 = [[val * 1e6 for val in lst] for lst in Y_values]
N_values_cm3 = [[val * 1e6 for val in lst] for lst in N_values]
...
# Plotting
plt.figure(figsize=(10, 6))
for bin_label, Y_data in enumerate(Y_values_cm3, start=3):
    plt.plot(x_values, Y_data, label=f'Bin{bin_label}_Y')

plt.xlabel('Seconds after midnight',  fontsize=16, fontweight='bold')
plt.ylabel('Droplet concentration (dN/dlogd) /cm^3',  fontsize=16, fontweight='bold')
plt.legend(fontsize=3.2, loc='upper right')
plt.grid(True)
plt.yscale('log')
plt.xlim(BCB_start, BCB_stop)  # Set the limits of the x-axis
    ...
    if BCB_start_list and BCB_stop_list:
        ...
        for idx, t in enumerate(times):
            if BCB_start <= int(t) <= BCB_stop:
                ...
                if 0 <= lwc_val <= 0.0000025 and 0 <= effec_val <= 0.00002:
                    val_Y.append(lwc_val)
                else:
                    val_N.append(lwc_val)
    
        lwc_FCDP_BCB.append({'Date': date, 'Val_Y':val_Y, 'Val_N': val_N})
        break
...
for entry in lwc_FCDP_BCB:
    lwc_Y_values.extend(entry['Val_Y'])
...
plt.show()
#%%
# Initialize an empty list to store the LWC values
lwc_Y_values = []

# Iterate over the data in lwc_FCDP_BCB
for entry in lwc_FCDP_BCB:
    lwc_Y_values.extend(entry['Val_Y'])

# Convert LWC values from kg/m^3 to g/m^3
lwc_Y_values_g_m3 = [val * 1000 for val in lwc_Y_values]

# Find the maximum length among LWC list
max_length = len(lwc_Y_values_g_m3)

# Create x-axis values from BCB_start to BCB_stop
x_values = list(range(BCB_start, BCB_start + max_length))

# Plotting
plt.plot(x_values, lwc_Y_values_g_m3, label='FCDP_LWC', color='blue')
...
for index_data in time_FCDP_BCB_full_modified:
    ...
    for idx in range(len(index_data['Bin003_Y'])):  # Assuming 'Bin00_Y' has the same length as other bins
        ...
        for bin_label in range(18):
            ...
            bin_center_value = bin_radius[bin_label]
            
            # Calculate A and F for each bin
            A = (N_value * C_values[bin_label])
            B = (bin_center_value ** 3) / 1e18
            F = (A * ((4 / 3) * 3.14) * B ) * 1000
...
plt.xlabel('Calculated lwc kg/m^3', fontsize=16, fontweight='bold')
plt.ylabel('FCDP LWC kg/m^3', fontsize=16, fontweight='bold')
plt.legend(fontsize=12)
plt.yscale('log')
plt.xscale('log')
plt.grid(True)
...
sums_list_g_m3 = [val * 1000 for val in sums_list]

plt.plot(sums_list, lwc_Y_values_g_m3, label='FCDP_LWC', color='blue')
plt.xlabel('Calculated lwc (g/m^3)', fontsize=16, fontweight='bold')
plt.ylabel('FCDP LWC (g/m^3)', fontsize=16, fontweight='bold')
plt.legend(fontsize=12)
plt.yscale('log')
plt.xscale('log')
plt.grid(True)
...
plt.plot(x_values, sums_list_g_m3, label='Calculated LWC', color='red')  # Add this line
...
#Time series for one leg of 2/26 where the effective diameter for clear skies is 
# is plotted against the start time 48862 and the stop time 49061.  
#This time series runs from 48862 to 49061 and is for a Below Cloud Base leg. 
#%%
LWC_FCDP_BCB = []
        ...
        for idx, t in enumerate(times):
            if BCB_start <= int(t) <= BCB_stop:
                ...
                if 0 <= lwc_val <= 0.0000025 and 0 <= effec_val <= 0.00002:
                    ...
                    EV_Y.append(effec_val)



                else:
                    LWC_N.append(lwc_val)
                    EV_N.append(effec_val)
                   
                    print("Added to LWC_Y. Corresponding effective diameter value:", effec_val)
                    print("Length of LWC_Y after adding value:", len(LWC_Y))
                    print("Length of EV_Y after adding value:", len(EV_Y))
                                    
        
        lwc_FCDP_BCB.append({'Date': date, 'LWC_Y': LWC_Y, 'EV_Y': EV_Y, 'LWC_N': LWC_N, 'EV_N': EV_N})
        break

#%%
print("Final length of LWC_Y:", len(LWC_Y))
print("Final length of EV_Y:", len(EV_Y))
print("Final length of LWC_N:", len(LWC_N))
print("Final length of EV_N:", len(EV_N))
# %%

#%%
# Convert effective diameter values to effective radius by dividing by 2
EV_Y_radius = [ev_val / 2 for ev_val in EV_Y]
EV_N_radius = [ev_val / 2 for ev_val in EV_N]

# Print out the lengths of EV_Y and EV_N after conversion
print("Length of EV_Y after conversion to radius:", len(EV_Y_radius))
print("Length of EV_N after conversion to radius:", len(EV_N_radius))

lwc_FCDP_BCB.append({'Date': date, 
                     'LWC_Y': LWC_Y, 
                     'EV_Y': EV_Y, 
                     'EV_Y_radius': EV_Y_radius, 
                     'LWC_N': LWC_N, 
                     'EV_N': EV_N, 
                     'EV_N_radius': EV_N_radius})
print("Length of EV_Y_radius after appending:", len(EV_Y_radius))


# %%
max_length = len(EV_Y_radius)

# Create x-axis values from BCB_start to BCB_stop
x_values = list(range(BCB_start, BCB_start + max_length))

# plt.figure(figsize=(10, 6))
plt.plot(x_values, EV_Y_radius, label='FCDP_ER', color='red')
plt.plot(x_values, lwc_Y_values_g_m3, label='FCDP_LWC', color='green')

plt.xlabel('Seconds after midnight', fontsize=16, fontweight='bold')
plt.ylabel('Effective radius BCB (m^3)\n versus LWC g/m^3', fontsize=16, fontweight='bold')
plt.legend(fontsize=12)
plt.xlim(BCB_start, BCB_stop)  # Set the limits of the x-axis
plt.title('2022-02-26\n 48862 - 49061 seconds after midnight\n Clear sky', fontsize=16, fontweight='bold')
plt.show()
...
#%%
# %%
# Rewrite the code to plot EV_Y_radius against x_values
plt.plot(x_values, EV_Y_radius, label='FCDP_ER', color='blue')

plt.xlabel('Seconds after midnight', fontsize=16, fontweight='bold')
plt.ylabel('Effective Radius (m)', fontsize=16, fontweight='bold')
plt.legend(fontsize=12)
plt.xlim(BCB_start, BCB_stop)  # Set the limits of the x-axis
plt.title('2022-02-26\n 48862 - 49061 seconds after midnight\n Clear sky', fontsize=16, fontweight='bold')
plt.show()
#%%
plt.plot(EV_Y_radius, LWC_Y, label='FCDP_ER', color='blue')
plt.xlabel('Effective Radius (m)', fontsize=16, fontweight='bold')
plt.ylabel('Liquid Water Content (g/m^3)', fontsize=16, fontweight='bold')
plt.title('2022-02-26\n 48862 - 49061 seconds after midnight\n Clear sky', fontsize=16, fontweight='bold')
...
#%%
plt.plot(EV_N_radius, LWC_N, label='FCDP_ER', color='blue')
plt.xlabel('Effective Radius (m)', fontsize=16, fontweight='bold')
plt.ylabel('Liquid Water Content (g/m^3)', fontsize=16, fontweight='bold')
plt.title('2022-02-26\n 48862 - 49061 seconds after midnight\n Cloduy sky', fontsize=16, fontweight='bold')

plt.grid(True)
plt.show()
#%%
plt.plot(EV_Y_radius, lwc_Y_values_g_m3, label='FCDP_ER', color='blue')
plt.xlabel('Effective Radius (m)', fontsize=16, fontweight='bold')
plt.ylabel('Liquid Water Content (g/m^3)', fontsize=16, fontweight='bold')
plt.title('2022-02-26\n 48862 - 49061 seconds after midnight\n Clear sky', fontsize=16, fontweight='bold')
...
ER_volume_Y = [(4/3) * math.pi * (radius ** 3) for radius in EV_Y_radius]
ER_volume_N = [(4/3) * math.pi * (radius ** 3) for radius in EV_N_radius]

# Print out the lengths of ER_volume_Y and ER_volume_N
print("Length of ER_volume_Y:", len(ER_volume_Y))
print("Length of ER_volume_N:", len(ER_volume_N))

# Append ER_volume_Y and ER_volume_N to lwc_FCDP_BCB
lwc_FCDP_BCB.append({
    'Date': date, 
    'LWC_Y': LWC_Y, 
    'EV_Y': EV_Y, 
    'EV_Y_radius': EV_Y_radius, 
    'ER_volume_Y': ER_volume_Y, 
    'LWC_N': LWC_N, 
    'EV_N': EV_N, 
    'EV_N_radius': EV_N_radius, 
    'ER_volume_N': ER_volume_N
})

# Print out the lengths again to ensure they haven't changed
print("Length of ER_volume_Y after appending:", len(ER_volume_Y))
print("Length of ER_volume_N after appending:", len(ER_volume_N))
#%%

plt.plot(ER_volume_Y, lwc_Y_values_g_m3, label='FCDP_ER', color='blue')
plt.xlabel('Effective volume (m^3)', fontsize=16, fontweight='bold')
plt.ylabel('Liquid Water Content (g/m^3)', fontsize=16, fontweight='bold')
plt.title('2022-02-26\n 48862 - 49061 seconds after midnight\n Clear sky', fontsize=16, fontweight='bold')
...
# %%
max_length = len(ER_volume_Y)

# Create x-axis values from BCB_start to BCB_stop
x_values = list(range(BCB_start, BCB_start + max_length))

# plt.figure(figsize=(10, 6))
plt.plot(x_values, ER_volume_Y, label='FCDP_ER', color='red')
plt.plot(x_values, lwc_Y_values_g_m3, label='FCDP_LWC', color='green')

plt.xlabel('Seconds after midnight', fontsize=16, fontweight='bold')
plt.ylabel('Effective Volume BCB (m^3)\n versus LWC g/m^3', fontsize=16, fontweight='bold')
plt.legend(fontsize=12)
plt.xlim(BCB_start, BCB_stop)  # Set the limits of the x-axis
plt.title('2022-02-26\n 48862 - 49061 seconds after midnight\n Clear sky', fontsize=16, fontweight='bold')
plt.show()
